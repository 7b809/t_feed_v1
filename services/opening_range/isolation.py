...
from .candle_utils import (get_contract_info_by_key, get_live_ema_calculation_mode_text, get_now_market_time, is_option_contract, normalize_option_type, parse_candle_timestamp, safe_float)
from .constants import (
    DEFAULT_LIVE_EMA_CALCULATION_MODE, DEFAULT_MAIN_INDEX_KEY, DEFAULT_STRIKE_FROM, DEFAULT_STRIKE_TO,
    get_isolated_notify_enabled, get_isolation_allow_backfill_touch, get_isolation_allow_live_touch,
    get_isolation_enabled, get_isolation_options_only, get_isolation_priority_levels, get_isolation_touch_levels,
    get_isolation_window_points
)

logger = get_logger(__file__)

def _format_numeric_value(value: Any, unavailable_text: str = "not_available") -> str:
    if value is None: return unavailable_text
    try:
...
def get_reference_opening_range_average() -> float | None:
    with runtime_state.opening_range_cache_lock:
        ...
        cached_latest_ltp = runtime_state.opening_range_cache.get("latest_main_index_ltp")
    if cached_latest_ltp is not None:
        cached_latest_ltp_value = safe_float(cached_latest_ltp, default=0.0)
...
def build_average_window(reference_average: float) -> dict:
    normalized_reference_average = safe_float(reference_average, default=0.0)
    ...
    window_points = get_isolation_window_points()
...
def is_event_eligible_for_isolation(event: dict) -> tuple[bool, str]:
    ...
    if not (average_window["final_lower"] <= strike_value <= average_window["final_upper"]):
...
def choose_best_isolation_event(events: list) -> dict | None:
    ...
    for event_index, event in enumerate(events):
        ...
        if not eligible:
            logger.debug("Isolation candidate rejected. reason=%s, instrument_key=%s, level=%s", reason,
        ...
        if not isinstance(contract_info, dict):
            contract_info = {}
        strike = safe_float(contract_info.get("strike_price"), default=0.0)
        level = str(event.get("level") or "").strip().upper()
        parsed_touch_time = parse_candle_timestamp(event.get("touch_time"))
        touch_sort_value = parsed_touch_time.timestamp() if parsed_touch_time is not None else float(event_index)
    ...
    if not eligible_events:
        return None
    selected = min(eligible_events, key=lambda item: (item["priority"], item["distance_to_average"], item["touch_sort_value"], item["strike"]))
...
def should_replace_isolated_instrument(new_event: dict) -> bool:
    ...
    if not current_state.get("selected"):
        logger.info("Isolation selection allowed. reason=no_existing_selection, new_instrument_key=%s, new_level=%s",
                    new_instrument_key, new_event.get("level"))
        return True
    logger.info("Isolation selection blocked. reason=instrument_already_locked_for_market_day, current_instrument_key=%s, current_level=%s, new_instrument_key=%s, new_level=%s",
                current_state.get("instrument_key"), current_state.get("selected_level"),
                new_instrument_key, new_event.get("level"))
    return False
...
def format_isolated_instrument_title(selected_state: dict) -> str:
    ...
    if not isinstance(contract_info, dict):
        contract_info = {}
    strike = _format_numeric_value(contract_info.get("strike_price"), unavailable_text="N/A")
    option_type = normalize_option_type(contract_info.get("instrument_type") or contract_info.get("option_type"))
...
def send_isolated_instrument_notification(selected_state: dict) -> bool:
    ...
    trigger_price = _format_numeric_value(selected_state.get("trigger_price"), unavailable_text="N/A")
    level_value = _format_numeric_value(selected_state.get("level_value"), unavailable_text="N/A")
    nifty_ltp = _format_numeric_value(selected_state.get("latest_main_index_ltp"), unavailable_text="not_available")
    short_touch_time = _get_short_market_time(selected_state.get("touch_time"))
    message = (f"{strike} {option_type_text} selected after {selected_level} touch\n\n"
    ...
    try:
        return bool(telegram_service.send_message(title="Opening Range Instrument Isolated", message=message, level="OPENING_RANGE"))
    except Exception as ex:
        logger.error("Failed sending isolated-instrument Telegram notification. instrument_key=%s, error=%s: %s",
                     selected_state.get("instrument_key"), type(ex).__name__, ex)
        return False
...
def isolate_instrument_from_event(event: dict) -> bool:
    ...
    eligible, reason = is_event_eligible_for_isolation(event)
    if not eligible:
        logger.info("Isolation skipped. reason=%s, instrument_key=%s, level=%s", reason, event.get("instrument_key"), event.get("level"))
        return False
    if not should_replace_isolated_instrument(event):
        logger.info("Isolation selection skipped. instrument_key=%s, level=%s, reason=instrument_already_selected_for_market_day",
                    event.get("instrument_key"), event.get("level"))
        return False
    instrument_key = str(event.get("instrument_key") or "").strip()
    if not instrument_key:
        return False
    ...
    reference_average = get_reference_opening_range_average()
    average_window = build_average_window(reference_average) if reference_average is not None else None
    with runtime_state.opening_range_cache_lock:
        cache_data = runtime_state.opening_range_cache.get("data", {})
        if not isinstance(cache_data, dict):
            cache_data = {}
    ...
    new_selected_state = {
        ...
        "selection_reason": "initial_level_priority_nearest_to_opening_range_average_daily_lock",
        ...
        "last_ema_alert": None,
        "disabled": False,
        "message": "Opening Range instrument isolated and locked for EMA Telegram alerts for the market day.",
    }
    with runtime_state.selected_or_lock:
        if runtime_state.selected_or_instrument_state.get("selected"):
            logger.info("Isolation state update blocked. reason=instrument_selected_by_another_event, current_instrument_key=%s, new_instrument_key=%s, new_level=%s",
                        runtime_state.selected_or_instrument_state.get("instrument_key"), instrument_key, event.get("level"))
            return False
    ...
    logger.info("Opening Range isolated instrument selected and locked for market day. instrument_key=%s, level=%s, strike=%s, type=%s, reference_average=%s, window_points=%s",
                instrument_key, new_selected_state.get("selected_level"), contract_info.get("strike_price"),
                normalize_option_type(contract_info.get("instrument_type") or contract_info.get("option_type")) or contract_info.get("instrument_type"),
                reference_average, average_window.get("window_points") if isinstance(average_window, dict) else None)
    ...
    if notification_enabled and not notification_sent:
        logger.warning("Instrument was isolated but its Telegram notification was not delivered. instrument_key=%s", instrument_key)
    return True
...
def try_isolate_from_touch_events(events: list) -> bool:
    ...
    with runtime_state.selected_or_lock:
        if runtime_state.selected_or_instrument_state.get("selected"):
            logger.debug("Isolation event processing skipped. reason=instrument_already_selected_for_market_day, instrument_key=%s",
