...
async def get_opening_range_latest_status():
    ...
    - Opening Range levels are calculated for every subscribed instrument.
    - Backfill and live touch monitoring check R2/R3/S2/S3 touches.
    - One instrument can be isolated based on:
        1. Opening Range average +/- configured window.
        2. Clamp inside configured STRIKE_FROM and STRIKE_TO.
        3. R3/S3 priority before R2/S2.
        4. Nearest strike to Opening Range average.
    - Live EMA continues for all initialized instruments.
    - EMA calculation source depends on LIVE_EMA_CALCULATION_MODE.
...
async def trigger_opening_range_fetch(
    ...
    - Scans post-OR candles for already touched R2/R3/S2/S3.
    - Stores results in memory for every subscribed instrument.
    - Selects or updates isolated instrument when eligible touch exists.
    - Saves to data/opening_range_results.json if enabled.

    Current flow:
    - Live EMA continues for all initialized instruments.
    - EMA calculation source depends on LIVE_EMA_CALCULATION_MODE.
    - Telegram EMA alert is sent only for the isolated instrument.
...
async def get_opening_range_full_cache():
    ...
    - It includes isolated instrument state and isolated EMA alert counters.
    """
...
async def get_opening_range_ema_context(
    ...
    - Opening Range EMA payload enrichment.
    - Per-instrument touch status.
...
async def fetch_opening_range_for_single_instrument(
    ...
    It also returns backfill R2/R3/S2/S3 touch events for that instrument.
    """
...
async def get_selected_opening_range_instrument():
    ...
    Current flow:
    - Selected OR route now returns isolated instrument state.
    - Isolated instrument is chosen from R2/R3/S2/S3 touch logic.
...
async def get_selected_opening_range_ema_alerts(
    ...
    Current flow:
    - Returns isolated instrument EMA Telegram alert records.
    - Alerts are generated only for the isolated instrument.
    """
...
async def get_opening_range_pending_touch_events_route():
    ...
    Current flow:
    - Legacy grouped touch Telegram is disabled by default.
    - Isolated instrument selection notification is separate.
    - Isolated instrument EMA alerts are separate.
    """
...
async def flush_opening_range_pending_touch_events_route(
    ...
    Recommended behavior keeps this disabled.
    """
