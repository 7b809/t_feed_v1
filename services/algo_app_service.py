...
class AlgoAppService:
    ...
    def _get_simulation_metadata(
        ...
        is_simulation = bool(
            ...
            or simulation_data.get("enabled")
            or event_type.startswith("simulated_")
        )
    ...
    def _validate_simulation_delivery(
        self,
        payload: dict,
    ) -> tuple[bool, str | None]:
        metadata = self._get_simulation_metadata(payload)

        if not metadata["is_simulation"]:
            return True, None
    ...
    def _build_headers(
        ...
        simulation_metadata = self._get_simulation_metadata(payload)
    ...
    def _build_result(
        ...
        *,
        ...
        response: Any = None,
        error: str | None = None,
        telegram_notification: dict | None = None,
    ) -> dict:
        return {
        ...
        for attempt_number in range(
            ...
            try:
                ...
                if 200 <= response.status_code < 300:
                    telegram_sent = self._send_algo_app_success_telegram(
                        event_id=event_id
                    )
