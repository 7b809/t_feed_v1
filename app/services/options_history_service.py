...
class OptionsBatchHistoryService:
    ...
    def _fetch_single_contract_history(
        ...
        """
        Worker executed inside thread pool.
        """
        ...
        try:
            ...
            if not result or result.get("total_candles", 0) == 0:
                ...
                logger.warning(
                    f"No candles retrieved for " f"{trading_symbol} ({instrument_key})"
                )
            ...
            # --------------------------------------------------
            # IMPORTANT:
            # Store candle data for EMA initialization
            # --------------------------------------------------

            candles = list(self.history_service.candles)
            ...
            logger.error(
                ...
                f"{trading_symbol} ({instrument_key}): {ex}"
            )
    ...
    def process_target_options_history(
        ...
        """
        Processes option history for configured strike range.
        """
        ...
        if not data:
            ...
            logger.warning(
                "Options cache is empty. " "Cannot run historical processing."
            )

            return {
                "status": "error",
                "message": "Options cache is empty.",
            }
        ...
        logger.info(
            f"Starting historical processing for "
            f"{total_targets} option contracts "
            f"(Strikes {min_strike} to {max_strike})"
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_symbol = {
                executor.submit(
                    self._fetch_single_contract_history,
                    contract,
                    save_files,
                ): contract.get(
                    "trading_symbol",
            ...
            for future in as_completed(future_to_symbol):
                ...
                try:
                    ...
                    logger.error(
                        f"Execution error loading " f"history for {symbol}: {ex}"
                    )
                ...
                if (
                    ...
                    logger.info(f"Progress: " f"{completed_tasks}/" f"{total_targets}")

        logger.info(
            f"Historical processing complete. "
            f"Successfully processed "
            f"{processed_count}/{total_targets}"
        )
