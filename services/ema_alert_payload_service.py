...
def get_suggested_order_side(
    cross_type: Any, isolated_instrument_type: Any
) -> str | None:
...
def normalize_timestamp(value: Any) -> str | None:
    ...
    if isinstance(value, date):
        return datetime.combine(
            value, datetime.min.time(), tzinfo=market_timezone
        ).isoformat()
...
def build_event_id(
    instrument_key: str, direction: str, event_timestamp: Any = None
) -> str:
...
def normalize_candle_payload(
    candle: Any, fallback_event: dict | None = None
) -> dict | None:
    ...
    close_price = safe_float(
        candle.get("close", fallback_event.get("close", fallback_event.get("ltp")))
    )
    volume = safe_float(
        candle.get("volume", candle.get("vol", fallback_event.get("volume")))
    )
    timestamp = (
...
def build_opening_range_payload(selected_state: dict, ema_event: dict) -> dict:
    ...
    if not isinstance(event_opening_range, dict):
        event_opening_range = {}
    ...
    event_levels = (
        ...
        if isinstance(event_opening_range.get("levels"), dict)
    ...
    return {
        ...
        or ema_event.get("selected_level"),
        ...
        "touch_source": selected_state.get("touch_source")
...
def build_ema_payload(ema_event: dict, ema_candle: dict, alert_direction: str) -> dict:
    ...
    calculation_mode = (
        str(
            ema_event.get("ema_calculation_mode", get_live_ema_calculation_mode())
    ...
    fast_period = safe_int(
        ema_event.get("ema_fast_period", getattr(config, "LIVE_EMA_FAST_PERIOD", 9)),
        default=getattr(config, "LIVE_EMA_FAST_PERIOD", 9),
    )
    slow_period = safe_int(
        ema_event.get("ema_slow_period", getattr(config, "LIVE_EMA_SLOW_PERIOD", 21)),
        default=getattr(config, "LIVE_EMA_SLOW_PERIOD", 21),
    )
    return {
        ...
        "timestamp": ema_candle.get("timestamp")
...
def normalize_order_instrument(instrument: dict) -> dict:
    ...
    live_ltp = safe_float(
        instrument.get("live_ltp"), default=safe_float(instrument.get("ltp"))
    )
    ...
    return {
        ...
        "live_ltp_updated_at": normalize_timestamp(
        ...
        "ema_candle_low": safe_float(
            ...
            default=safe_float(candle.get("low")) if isinstance(candle, dict) else None,
        ),
    ...
    return {
        ...
        "budget_filter": {
            ...
            "maximum_instruments": safe_int(
                getattr(config, "EMA_ALERT_BUDGET_MAX_INSTRUMENTS", 2), default=2
            ),
            "sort_mode": str(
                getattr(
            ...
            "range_inclusive": bool(
                getattr(config, "EMA_ALERT_BUDGET_RANGE_INCLUSIVE", True)
            ),
...
def build_isolated_ema_alert_payload(
    ...
    if not isinstance(selected_contract_info, dict):
        selected_contract_info = {}
    instrument_key = str(
    ...
    direction = (
        ...
        if alert_direction
            ...
            ema_event.get("cross_type"), ema_event.get("current_signal")
        )
    )
    resolved_order_side = normalize_option_type(
        suggested_order_option_type
    ) or get_suggested_order_side(ema_event.get("cross_type"), resolved_isolated_type)
    ...
    payload = {
        "schema_version": str(
            getattr(config, "ALGO_APP_PAYLOAD_SCHEMA_VERSION", "1.0")
        ),
        "event_id": (
            event_id
            if bool(getattr(config, "ALGO_APP_INCLUDE_EVENT_ID", True))
            else None
        ),
        ...
        "opening_range": build_opening_range_payload(
            selected_state=selected_state, ema_event=ema_event
        ),
        "market_snapshot": {
        ...
        "ema": build_ema_payload(
            ema_event=ema_event, ema_candle=resolved_candle, alert_direction=direction
        ),
        ...
        "raw_ema_event": build_safe_raw_ema_event(ema_event),
    }
    ...
    if not bool(
        ...
        nearest_instruments = payload.get("order_suggestion", {}).get(
            "nearest_instruments", []
        )
...
class EmaAlertPayloadService:
    ...
    def get_suggested_order_side(
        self, cross_type: Any, isolated_instrument_type: Any
    ) -> str | None:
        return get_suggested_order_side(
            cross_type=cross_type, isolated_instrument_type=isolated_instrument_type
        )
