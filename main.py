...
from api.update_project_api import router as update_project_router
...
def get_ema_order_side_rule_text() -> str:
    return (
        "EMA Order Side Rule: bullish_cross uses the same option "
        "side as the isolated instrument; bearish_cross uses the "
        "opposite option side of the isolated instrument."
    )
...
def get_budget_range_status_text() -> str:
    ...
    return (
        f"Budget Range Enabled: {budget_enabled}\n"
        f"Budget Range: {budget_minimum:.2f} to "
        f"{budget_maximum:.2f}\n"
        f"Budget Maximum Instruments: {maximum_instruments}\n"
        f"Budget Sort Mode: {config.EMA_ALERT_BUDGET_SORT_MODE}"
    )
...
def get_runtime_config_status_text() -> str:
    try:
        ...
        return (
            f"Runtime Config Enabled: {status.get('enabled')}\n"
            f"Runtime Config Initialized: "
            f"{status.get('initialized')}\n"
            f"Runtime Config MongoDB Connected: "
            f"{status.get('database_connected')}\n"
            f"Runtime Config Overrides: "
            f"{status.get('override_count', 0)}\n"
            f"Isolation Window Points: "
            f"{get_isolation_window_points()}"
        )
    except Exception as ex:
        return (
            "Runtime Config Status: unavailable\n"
            f"Runtime Config Error: {type(ex).__name__}: {ex}"
        )
    ...
    try:
        ...
        telegram_service.send_message(
            ...
            message=(
                ...
                f"From Date: "
                f"{history_summary.get('from_date')}\n"
                f"To Date: "
                f"{history_summary.get('to_date')}\n"
                f"Interval: "
                f"{history_summary.get('interval')}\n"
                f"Total Instruments: "
                ...
                f"{get_runtime_config_status_text()}\n\n"
...
def run_daily_opening_range_fetch():
    ...
    try:
        ...
        if not subscribed_keys:
            warning_message = (
                "Opening range fetch skipped. No subscribed "
                "instruments found in options_cache."
            )
            logger.warning(warning_message)
            telegram_service.send_message(
        ...
        logger.info(
            ...
            "status=%s, total_instruments=%s, success=%s, "
            "empty=%s, insufficient_data=%s, failed=%s, "
            "backfill_touch_events=%s, "
            "latest_main_index_ltp=%s, "
            "isolated_selected=%s, "
            "isolated_instrument_key=%s, isolated_level=%s, "
            "output_file=%s",
...
def _is_todays_opening_range_already_saved() -> bool:
    ...
    try:
        ...
        logger.warning(
            "Could not verify today's saved Opening Range "
            "result. Startup catch-up will continue. "
            "error=%s: %s",
            type(ex).__name__,
            ex,
        )
...
def initialize_application_runtime_config() -> dict:
    ...
    try:
        ...
        logger.info(
            ...
            "database_connected=%s, overrides=%s, error=%s",
            status.get("enabled"),
            status.get("initialized"),
            status.get("database_connected"),
            status.get("override_count"),
            status.get("error"),
        )

        if status.get("error"):
            telegram_service.send_message(
                title="Runtime Configuration Warning",
                message=(
                    "Runtime configuration started with a "
                    "fallback condition.\n\n"
                    ...
                    "Environment/default values will be used "
                    "where runtime overrides are unavailable."
                ),
                level="WARNING",
            )
        else:
            telegram_service.send_message(
                title=("Runtime Configuration Initialized"),
                message=(
                    f"{get_runtime_config_status_text()}\n\n"
                    "Telegram runtime configuration is "
        ...
        telegram_service.send_exception_message(
            title=("Runtime Configuration Initialization Failed"),
            exception=ex,
            context=("initialize_application_runtime_config"),
        )
        ...
        return {
            "enabled": bool(config.RUNTIME_CONFIG_ENABLED),
            "initialized": False,
...
def run_initial_startup():
    ...
    try:
        ...
        logger.info(
            "Budget Maximum Instruments: %s",
            budget_maximum_instruments,
        )
        logger.info(
            "Isolation Window Points: %s",
            isolation_window_points,
        )
        ...
        startup_details = (
            ...
            f"{budget_maximum_instruments}\n"
            f"Isolation Window Points: "
            f"{isolation_window_points}\n"
            f"Algo App Enabled: "
...
def run_daily_market_hard_refresh():
    ...
    try:
        ...
        if not subscribed_keys:
            error_message = (
                "Daily hard refresh failed: No subscribed "
                "keys after contract reload."
            )
            logger.error(error_message)
            telegram_service.send_daily_refresh_message(
                success=False,
            ...
            def restart_done_callback(
                ...
                try:
                    ...
                    telegram_service.send_daily_refresh_message(
                        success=False,
                        error=(
                            "Streamer restart failed: " f"{type(ex).__name__}: {ex}"
                        ),
                    )
...
def start_scheduler() -> BackgroundScheduler:
    ...
    scheduler_message = (
        ...
        f"{config.TELEGRAM_RUNTIME_CONFIG_ENABLED}\n"
...
app.include_router(update_project_router)