...
def is_historical_candle_enabled() -> bool:
    ...
    If HISTORICAL_CANDLE_ENABLED is not configured,
...
def is_test_flag_enabled() -> bool:
    ...
    EMA crossover result file is saved only when TEST_FLAG=True
    and save_data=True.
    """
...
def should_fetch_intraday_today(now_market_time: datetime | None = None) -> bool:
    ...
    Logic:
    - If today is Saturday or Sunday, do not fetch intraday.
    - If current market time is before market open time, do not fetch intraday.
    - If current market time is market open time or later, fetch today's intraday candles.

    Default market open boundary:
        09:15 AM Asia/Kolkata
    """
...
def get_intraday_unit_and_interval(interval: str) -> tuple[str | None, str | None]:
    ...
    Upstox sample:
        get_intra_day_candle_data("NSE_EQ|INE848E01016", "minutes", "1")
    """
    ...
    logger.warning(
        f"Intraday candle fetch not supported for interval={interval}. "
        "Supported examples: 1minute, 3minute, 5minute, 15minute, 30minute."
    )
...
def get_default_history_range(
    ...
    New logic:
    - Before 09:15 AM IST:
        Fetch historical API for previous `history_days` days ending yesterday.
        Do not fetch intraday today.

    - At/after 09:15 AM IST:
        Fetch historical API for previous `history_days - 1` days ending yesterday.
        Fetch today's candles using intraday API.
        This keeps total logical coverage around `history_days`, including today.

    Returns:
        from_date, to_date, intraday_today_required
    """
    ...
    logger.info(
        f"Historical date range decided. "
        f"market_time={now_market_time.strftime('%Y-%m-%d %H:%M:%S %Z')}, "
        f"intraday_today_required={intraday_today_required}, "
        f"historical_from_date={format_date(historical_from_date)}, "
        f"historical_to_date={format_date(historical_to_date)}, "
        f"history_days={history_days}, "
        f"historical_days={historical_days}"
    )
...
def calculate_ema_crossovers(
    ...
    if candles_count < min_required:
        ...
        return {
            ...
            "message": (
                f"Need at least {min_required} candles for EMA calculation. "
                f"Available candles: {candles_count}."
            ),
            "candles_count": candles_count,
            "latest_timestamp": latest_candle[0] if latest_candle else None,
    ...
    if len(closes) < min_required:
        return {
            "status": "insufficient_data",
            "message": (
                f"Need at least {min_required} valid close prices. "
                f"Available close prices: {len(closes)}."
            ),
            "candles_count": candles_count,
...
def save_ema_cross_results_to_file(
    ...
    Important:
    Raw historical candles are not saved.
    """
...
def initialize_live_ema_from_history(summary: dict) -> bool:
    ...
    This is intentionally imported inside the function to avoid circular imports.
    """
    ...
    try:
        ...
        logger.info(
            "Live EMA service initialized successfully from historical EMA summary."
        )
        ...
        logger.warning(
            "Live EMA service file not found. "
            "Skipping live EMA initialization. "
            "Expected file: services/live_ema_service.py"
        )
        return False

    except Exception as ex:
        logger.error(
            f"Live EMA service initialization failed: {type(ex).__name__}: {ex}"
        )
        return False
...
def fetch_intraday_candles_for_instrument(
    ...
    Uses:
        HistoryV3Api().get_intra_day_candle_data(
            instrument_key,
            unit,
            interval
        )
    ...
    if not unit or not intra_interval:
        return {
            ...
            "candles": [],
            "candles_count": 0,
            "error": f"Unsupported intraday interval: {interval}",
        }
    ...
    try:
        ...
        logger.info(
            f"Intraday candle request: instrument_key={instrument_key}, "
            f"unit={unit}, interval={intra_interval}"
        )
        ...
        logger.info(
            f"Intraday candle completed: instrument_key={instrument_key}, "
        ...
        logger.error(
            f"ApiException in intraday candle fetch for {instrument_key}: {error_body}"
        )
        ...
        logger.error(
            f"Exception in intraday candle fetch for {instrument_key}: {error_message}"
        )
...
def fetch_historical_candles_for_instrument(
    ...
    New behavior:
    - Historical API fetches previous days.
    - If fetch_intraday_today=True, today's candles are fetched using HistoryV3Api.
    - Historical and intraday candles are merged.
    - Raw candles are not saved.
    """

    logger.info(
        f"Fetching candles for instrument_key={instrument_key}, "
        f"interval={interval}, from_date={from_date}, to_date={to_date}, "
        f"fetch_intraday_today={fetch_intraday_today}"
    )

    if not from_date or not to_date:
        default_from, default_to, default_intraday_required = get_default_history_range(
    ...
    for batch in batches:
        ...
        logger.info(
            f"Historical batch request: instrument_key={instrument_key}, "
            f"interval={interval}, from_date={batch_from}, to_date={batch_to}"
        )
        ...
        try:
            ...
            logger.info(
                f"Historical batch completed: instrument_key={instrument_key}, "
                f"from_date={batch_from}, to_date={batch_to}, "
                f"candles_count={len(candles)}"
            )
            ...
            error_message = (
                ...
                f"from_date={batch_from}, to_date={batch_to}: {error_body}"
            )
            ...
            error_message = (
                f"{type(ex).__name__} for {instrument_key}, "
                f"from_date={batch_from}, to_date={batch_to}: {ex}"
            )
    ...
    if fetch_intraday_today:
        ...
        if intraday_result.get("error"):
            errors.append(
                f"Intraday fetch error for {instrument_key}: "
        ...
        batch_results.append(
            {
                ...
                "candles_count": intraday_result.get("candles_count", 0),
...
def fetch_historical_candles_for_all_subscribed(
    ...
    New behavior:
    - Before 09:15 AM:
        Historical fetch only, ending yesterday.
    - At/after 09:15 AM:
        Historical fetch up to yesterday + intraday today's candles using HistoryV3Api.
    - Raw candles are not saved.
    - EMA crossover results are kept in memory.
    - Live EMA service is initialized from latest EMA values.
    """
    ...
    from_date_obj, to_date_obj, intraday_today_required = get_default_history_range(
        history_days
    )
    ...
    logger.info(
        "================ HISTORICAL + INTRADAY EMA CROSSOVER FETCH STARTED ================"
    )

    logger.info(
        f"Fetching candles and calculating EMA crossover for "
        f"{len(subscribed_keys)} instruments. "
        f"interval={interval}, historical_from_date={from_date}, "
        f"historical_to_date={to_date}, intraday_today_required={intraday_today_required}, "
        f"history_days={history_days}, max_workers={max_workers}, "
        f"ema_fast={DEFAULT_EMA_FAST_PERIOD}, ema_slow={DEFAULT_EMA_SLOW_PERIOD}"
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_instrument):
            ...
            logger.info(
                f"Historical + intraday EMA progress: {completed_count}/{total_instruments} "
            ...
            try:
                ...
                logger.error(
                    f"Historical + intraday EMA fetch failed for "
    ...
    if save_data and is_test_flag_enabled():
        try:
            ...
            logger.error(
                f"Failed saving EMA crossover results: {type(ex).__name__}: {ex}"
            )
            summary["ema_results_file_error"] = f"{type(ex).__name__}: {ex}"

    else:
        logger.info(
            "EMA crossover result file not saved. "
            f"save_data={save_data}, TEST_FLAG={is_test_flag_enabled()}"
        )
    ...
    logger.info(
        f"Historical + intraday EMA crossover fetch completed. "
        ...
        f"intraday_today_required={intraday_today_required}, "
        f"intraday_used_count={intraday_used_count}, "
        f"max_workers={max_workers}, "
        f"live_ema_initialized={live_ema_initialized}"
    )

    logger.info(
        "================ HISTORICAL + INTRADAY EMA CROSSOVER FETCH COMPLETED ================"
    )
