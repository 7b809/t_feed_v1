    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            cleaned_item, nested_removed_count = (
                remove_historical_alert_data(item)
            )
...
class FlexibleModel(BaseModel):
    ...
    @classmethod
    def sanitize_historical_alert_data(
        cls,
        value: Any,
    ) -> Any:
        ...
        cleaned_value, _ = remove_historical_alert_data(
            deepcopy(value)
        )
...
class EmaEvent(FlexibleModel):
    ...
    def validate_ema_cross(self):
        if self.current_signal == self.previous_signal:
            raise ValueError("current_signal and previous_signal must differ")
...
class RawEmaEvent(FlexibleModel):
    ...
    def validate_ema_cross(self):
        ...
        if (
            ...
            and self.contract_info.instrument_key != self.instrument_key
        ):
            raise ValueError(
                "contract_info.instrument_key must match " "instrument_key"
            )

        if self.info is not None and self.info.instrument_key != self.instrument_key:
            raise ValueError("info.instrument_key must match instrument_key")
...
class OrderInstrument(FlexibleModel):
    ...
    def populate_live_ltp(self):
        ...
        if self.market_data is not None and self.market_data.ltp is not None:
...
class OrderSuggestion(FlexibleModel):
    ...
    nearest_order_instruments: list[OrderInstrument] = Field(
        default_factory=list,
    )
    ...
    def validate_suggested_side(self):
        if self.budget_range is not None:
            budget_type = self.budget_range.instrument_type

            if budget_type is not None and budget_type != self.suggested_order_side:
                raise ValueError(
                    "suggested_order_side must match " "budget_range.instrument_type"
                )

        for instrument in self.get_candidate_instruments():
            if instrument.instrument_type != self.suggested_order_side:
                raise ValueError(
                    "candidate instrument type must match " "suggested_order_side"
                )
    ...
    def get_candidate_instruments(
        ...
        if self.budget_filter is not None and self.budget_filter.instruments:
            return self.budget_filter.instruments

        if self.budget_range is not None and self.budget_range.instruments:
...
class InstrumentSelection(FlexibleModel):
    ...
    def calculate_selection_values(self):
        ...
        if self.selected_live_ltp is None:
            self.selected_live_ltp = self.selected_instrument.live_ltp

        if self.selected_live_ltp is not None and self.target_price is not None:
            self.price_difference = round(
                abs(self.selected_live_ltp - self.target_price),
                2,
            )
...
class MarketOrderResult(FlexibleModel):
    ...
    response: Any | None = None
    error: Any | None = None
    selected_instrument: SelectedOrderInstrument | None = None
...
class OrderExecutionResult(FlexibleModel):
    ...
    place_order_result: MarketOrderResult | None = None
    error: str | None = None
    ...
    def validate_execution_result(self):
        if self.order_status == "IGNORED":
            if not self.reason:
                raise ValueError("reason is required when " "order_status is IGNORED")

            if self.exit_result is not None:
                raise ValueError("exit_result must be empty for " "an ignored order")

            if self.place_order_result is not None:
                raise ValueError(
                    "place_order_result must be empty " "for an ignored order"
                )

        if self.order_status == "ORDER_PLACED":
            if not self.success:
                raise ValueError(
                    "success must be true when " "order_status is ORDER_PLACED"
                )

            if self.exit_result is None or not self.exit_result.success:
                raise ValueError(
                    "Successful exit_result is required "
                    "when order_status is ORDER_PLACED"
                )

            if self.place_order_result is None or not self.place_order_result.success:
                raise ValueError(
        ...
        if self.order_status == "EXIT_FAILED":
            if self.success:
                raise ValueError(
                    "success must be false when " "order_status is EXIT_FAILED"
                )

            if self.place_order_result is not None:
                raise ValueError(
                    "place_order_result must be empty " "when position exit fails"
                )

        if self.order_status == "PLACE_ORDER_FAILED":
            if self.success:
                raise ValueError(
                    "success must be false when " "order_status is PLACE_ORDER_FAILED"
                )

            if self.exit_result is None:
                raise ValueError(
                    "exit_result is required when " "order placement fails"
                )

        if self.order_status == "PROCESSING_FAILED":
            if self.success:
                raise ValueError(
                    "success must be false when " "order_status is PROCESSING_FAILED"
                )
...
class OrderRequestProcessing(FlexibleModel):
    ...
    def validate_processing_result(self):
        if self.selected_instrument is not None:
            ...
            selected_ltp = self.selected_instrument.live_ltp

            if selected_ltp is not None:
                self.price_difference = round(
                    abs(selected_ltp - self.target_price),
                    2,
                )

        if self.execution_result is not None and self.selected_instrument is None:
            raise ValueError(
                "selected_instrument is required when " "execution_result exists"
            )
...
class Simulation(FlexibleModel):
    ...
    delivery_controls: SimulationDeliveryControls | None = None
...
class RawEmaOrderRequest(FlexibleModel):
    ...
    instrument_type: InstrumentType | None = None
    available_instruments: list[OrderInstrument] = Field(
        default_factory=list,
    )
    ...
    def validate_request_consistency(self):
        if (
            ...
            and self.instrument.instrument_key != self.raw_ema_event.instrument_key
        ):
            raise ValueError(
                "instrument.instrument_key must match " "raw_ema_event.instrument_key"
            )

        if self.ema is not None:
            if self.ema.cross_type != self.raw_ema_event.cross_type:
                raise ValueError(
                    "ema.cross_type must match " "raw_ema_event.cross_type"
                )

            if self.ema.current_signal != self.raw_ema_event.current_signal:
                raise ValueError(
                    "ema.current_signal must match " "raw_ema_event.current_signal"
                )

        if (
            self.processing is not None
            and self.processing.execution_result is not None
            and not self.event_id
        ):
            raise ValueError("event_id is required when " "execution_result exists")
