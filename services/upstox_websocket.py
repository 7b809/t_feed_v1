...
class UpstoxStreamer:
    def __init__(self):
        ...
        self.market_time_format = getattr(config, "MARKET_TIME_FORMAT", "%Y-%m-%d %H:%M:%S %Z")
    ...
    def _load_market_timezone(self) -> ZoneInfo:
        ...
        try:
            ...
            logger.error("Invalid MARKET_TIMEZONE configured: %s. Falling back to Asia/Kolkata.", timezone_name)
    ...
    def _should_process_incoming_message(self) -> bool:
        ...
        if bool(getattr(config, "EMA_ISOLATED_INSTRUMENT_TELEGRAM_ENABLED", True)):
            return True
        if bool(getattr(config, "ALGO_APP_ENABLED", False)):
            return True
        return False
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "isolated_ema_alert_accepted_count": self.isolated_ema_alert_accepted_count,
            "isolated_ema_alert_failed_count": self.isolated_ema_alert_failed_count,
            "selected_or_ema_alert_processed_count": self.selected_or_ema_alert_processed_count,
            "selected_or_ema_alert_sent_count": self.selected_or_ema_alert_sent_count,
            "selected_or_ema_alert_failed_count": self.selected_or_ema_alert_failed_count,
    ...
    async def start(self) -> None:
        logger.info("UpstoxStreamer.start() called. market_time=%s", self._now_market_time())
        if self.is_running:
        ...
        logger.info("UpstoxStreamer task created. market_time=%s, ema_mode=%s", self._now_market_time(), self._get_live_ema_calculation_mode_text())

    async def stop(self) -> None:
        logger.info("UpstoxStreamer.stop() called. market_time=%s", self._now_market_time())
        self.is_running = False
        if self.streamer:
            try:
                ...
                logger.error("Upstox streamer disconnect failed: %s: %s", type(ex).__name__, ex)
            finally:
                self.streamer = None
        if self.task:
            try:
                ...
                logger.error("Upstox task cancellation failed: %s: %s", type(ex).__name__, ex)
            finally:
                self.task = None

    async def restart(self) -> None:
        logger.info("UpstoxStreamer.restart() called. market_time=%s", self._now_market_time())
        try:
            await self.stop()
            ...
            logger.error("UpstoxStreamer restart failed: %s: %s", type(ex).__name__, ex)
            raise
    ...
    async def _run_loop(self) -> None:
        logger.info("Entered Upstox streamer loop. market_time=%s", self._now_market_time())
        while self.is_running:
            ...
            try:
                ...
                if not keys:
                    logger.warning("No subscribed instrument keys. Waiting 5 seconds.")
                    await asyncio.sleep(5)
                    continue
                self.streamer = upstox_client.MarketDataStreamerV3(api_client, keys, mode)

                def on_open():
                    logger.info("Connected to Upstox Market Stream V3. instruments=%s", len(keys))
                ...
                def on_message(message):
                    ...
                    if self.loop and self.loop.is_running():
                        future = asyncio.run_coroutine_threadsafe(self._process_message(message), self.loop)

                        def callback(completed_future):
                            try:
                                completed_future.result()
                            except Exception as ex:
                                logger.error("Message processing future failed: %s: %s", type(ex).__name__, ex)
                ...
                def on_error(error):
                    logger.error("Upstox WebSocket error: %s: %s", type(error).__name__, error)

                def on_close(close_status_code, close_message):
                    logger.warning("Upstox WebSocket closed. status=%s, message=%s", close_status_code, close_message)
                ...
                logger.error("Upstox connection exception: %s: %s. Reconnecting in 5 seconds.", type(ex).__name__, ex)
                await asyncio.sleep(5)
    ...
    def _enrich_ema_event_with_opening_range(self, instrument_key: str, live_ema_cross_event: dict) -> dict:
        ...
        if not bool(getattr(config, "EMA_CROSS_INCLUDE_OPENING_RANGE_LEVELS", True)):
            live_ema_cross_event.update({"opening_range": {}, "touch_status": {}, "latest_intraday_close": None, "latest_main_index_ltp": None, "processed_at": None, "isolated_instrument": {}})
            return live_ema_cross_event
        try:
            ...
            logger.error("EMA Opening Range enrichment failed. instrument_key=%s, error=%s: %s", instrument_key, type(ex).__name__, ex)
            live_ema_cross_event.update({"opening_range": {}, "touch_status": {}, "latest_intraday_close": None, "latest_main_index_ltp": None, "processed_at": None, "isolated_instrument": {}})
        return live_ema_cross_event
    ...
    def _process_isolated_ema_alert(self, live_ema_cross_event: dict) -> bool:
        if not isinstance(live_ema_cross_event, dict):
            return False
        telegram_enabled = bool(getattr(config, "EMA_ISOLATED_INSTRUMENT_TELEGRAM_ENABLED", True))
        algo_app_enabled = bool(getattr(config, "ALGO_APP_ENABLED", False))
        ...
        try:
            ...
            if accepted:
                ...
                logger.info("Isolated EMA alert accepted. instrument_key=%s, cross_type=%s, timestamp=%s, ema_mode=%s, telegram_enabled=%s, algo_app_enabled=%s", live_ema_cross_event.get("instrument_key"), live_ema_cross_event.get("cross_type"), live_ema_cross_event.get("timestamp"), live_ema_cross_event.get("ema_calculation_mode"), telegram_enabled, algo_app_enabled)
                return True
            logger.info("Isolated EMA event not accepted. instrument_key=%s, cross_type=%s", live_ema_cross_event.get("instrument_key"), live_ema_cross_event.get("cross_type"))
            return False
        except Exception as ex:
            self.isolated_ema_alert_failed_count += 1
            self.selected_or_ema_alert_failed_count += 1
            logger.error("Isolated EMA alert processing failed. instrument_key=%s, error=%s: %s", live_ema_cross_event.get("instrument_key"), type(ex).__name__, ex)
            return False

    def _process_isolated_ema_telegram_alert(self, live_ema_cross_event: dict) -> bool:
    ...
    async def _process_message(self, message) -> None:
        try:
            ...
            if isinstance(message, dict):
                ...
                logger.warning("Unsupported Upstox message type: %s", type(message))
                return
            feeds = tick_dict.get("feeds", {})
            if not isinstance(feeds, dict):
                logger.warning("Invalid Upstox feeds type: %s", type(feeds))
                return
            if not feeds:
                return
                ...
                try:
                    if bool(getattr(config, "OPENING_RANGE_TOUCH_ALERT_ENABLED", True)):
                        ...
                        if opening_range_touch_events:
                            ...
                            logger.info("Opening Range touch generated. instrument_key=%s, count=%s", instrument_key, len(opening_range_touch_events))
                except Exception as ex:
                ...
                if opening_range_touch_events and has_local_clients:
                    for opening_range_event in opening_range_touch_events:
                        try:
                            ...
                            logger.error("Opening Range broadcast failed. instrument_key=%s, error=%s: %s", instrument_key, type(ex).__name__, ex)
                if not has_local_clients:
                    continue
                try:
                    await broadcaster.broadcast_tick(instrument_key, tick_data, contract_info)
                    self.broadcast_success_count += 1
                except Exception as ex:
                    self.broadcast_failed_count += 1
                    logger.error("Tick broadcast failed. instrument_key=%s, error=%s: %s", instrument_key, type(ex).__name__, ex)
            try:
                if bool(getattr(config, "OPENING_RANGE_TOUCH_ALERT_ENABLED", True)):
                    sent = flush_pending_touch_alerts(force=False, source="live_tick")
                    if sent:
                        self.opening_range_alert_flush_count += 1
            except Exception as ex:
                logger.error("Opening Range alert flush failed: %s: %s", type(ex).__name__, ex)
        except json.JSONDecodeError as ex:
            logger.error("Upstox JSON decode failed: %s: %s", type(ex).__name__, ex)
        except Exception as ex:
            logger.error("Upstox message processing failed: %s: %s", type(ex).__name__, ex)
