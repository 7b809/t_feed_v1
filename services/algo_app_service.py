...
class AlgoAppService:
    ...
    def _get_response_body(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Failed reading Algo App response body. "
                    "status_code=%s, error=%s: %s",
    ...
    def _calculate_retry_delay(
        ...
        return self.retry_delay_seconds * max(
            1,
            attempt_number,
        )
    ...
    def _record_telegram_notification_result(
        self,
        event_id: str | None,
        success: bool,
        error: str | None = None,
    ...
    def _send_algo_app_success_telegram(
        self,
        event_id: str | None,
    ) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    "Algo App delivery confirmation " "sent to Telegram. event_id=%s",
                    event_id,
                )
    ...
    def _send_payload(
        ...
        if not self.is_configured():
            result = self._build_not_configured_result(
                event_id=event_id,
                started_at=started_at,
            )
        ...
        try:
            ...
            result = {
                ...
                "attempt_count": 0,
                "response": None,
                "error": (f"HeaderBuildError: " f"{type(ex).__name__}: {ex}"),
                "started_at": started_at,
                "completed_at": (self._now_market_time()),
            }
            ...
            logger.error(
                "Failed building Algo App request "
                "headers. event_id=%s, "
                "error=%s: %s",
                event_id,
                type(ex).__name__,
        ...
        for attempt_number in range(
            ...
            try:
                ...
                if 200 <= response.status_code < 300:
                    telegram_notification_sent = self._send_algo_app_success_telegram(
                        event_id=event_id
                    )
                    ...
                    result = {
                        ...
                        "telegram_notification": {
                            ...
                            "attempted": (self.telegram_success_notification_enabled),
                            "success": (telegram_notification_sent),
                        },
                        "started_at": started_at,
                        "completed_at": (self._now_market_time()),
                    }
                ...
                logger.warning(
                    "Algo App returned unsuccessful "
                    "HTTP response. event_id=%s, "
    ...
    def dispatch_ema_alert(
        ...
        try:
            ...
            if not self.enabled:
                logger.info(
                    "Algo App dispatch skipped because " "ALGO_APP_ENABLED=False."
                )
        ...
        try:
            ...
            if not valid_payload:
                logger.warning(
                    ...
                    "skipped because payload is invalid. "
                    "error=%s",
                    validation_error,
                )
        ...
        try:
            with self._lock:
                return {
                    ...
                    "telegram_notification_success_count": (
                        self.telegram_notification_success_count
                    ),
                    ...
                    "last_telegram_notification_at": (
                        self.last_telegram_notification_at
                    ),
                    "last_telegram_notification_event_id": (
                        self.last_telegram_notification_event_id
                    ),
                    "last_telegram_notification_success": (
                        self.last_telegram_notification_success
                    ),
