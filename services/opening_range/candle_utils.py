...
from .constants import (DEFAULT_LIVE_EMA_CALCULATION_MODE, DEFAULT_MARKET_OPEN_HOUR, DEFAULT_MARKET_OPEN_MINUTE,
                        DEFAULT_MARKET_TIMEZONE, DEFAULT_OPENING_RANGE_CANDLE_COUNT, DEFAULT_OPENING_RANGE_ENABLED)
...
def get_market_timezone() -> ZoneInfo:
    ...
    or cannot be loaded.
    """
    timezone_name = str(getattr(config, "MARKET_TIMEZONE", DEFAULT_MARKET_TIMEZONE) or DEFAULT_MARKET_TIMEZONE).strip()
...
def response_to_dict(api_response: Any) -> dict:
    ...
    if hasattr(api_response, "to_dict"):
        try:
            ...
            logger.debug("Could not convert API response using to_dict(). error=%s: %s", type(ex).__name__, ex)
    try:
...
def _parse_numeric_timestamp(timestamp_value: int | float | str) -> datetime | None:
    """
    ...
    try:
        return datetime.fromtimestamp(numeric_timestamp, tz=get_market_timezone())
    except (ValueError, OverflowError, OSError):
        return None

def parse_candle_timestamp(timestamp_value: Any) -> datetime | None:
    """
...
def normalize_candle(candle: list | tuple) -> dict | None:
    """
...
def normalize_candles(candles: list | tuple | None) -> list:
    """
    ...
    try:
        return sorted(normalized, key=lambda item: item["datetime"])
    except (KeyError, TypeError, ValueError):
        return normalized

def serialize_candle(candle: dict | None) -> dict:
    """
    ...
    if not isinstance(candle, dict):
        return {"timestamp": None, "open": None, "high": None, "low": None, "close": None, "volume": None, "oi": None}
    return {
...
def _get_candle_target_date(candles: list | tuple | None) -> date:
    """
...
def get_market_open_datetime(target_date: date | datetime | None = None) -> datetime:
    """
    ...
    if target_date is None:
        ...
        try:
            ...
            normalized_date = get_now_market_time().date()
    return datetime.combine(normalized_date, dt_time(hour=DEFAULT_MARKET_OPEN_HOUR, minute=DEFAULT_MARKET_OPEN_MINUTE), tzinfo=market_tz)

def get_opening_range_end_datetime(candle_count: int = DEFAULT_OPENING_RANGE_CANDLE_COUNT, target_date: date | datetime | None = None) -> datetime:
    """Returns the Opening Range completion datetime."""
    normalized_candle_count = max(1, safe_int(candle_count, default=DEFAULT_OPENING_RANGE_CANDLE_COUNT))
...
def select_post_opening_range_candles(candles: list, candle_count: int = DEFAULT_OPENING_RANGE_CANDLE_COUNT) -> list:
    ...
    if not isinstance(candles, list) or not candles:
        return []
    target_date = _get_candle_target_date(candles)
    opening_range_end_dt = get_opening_range_end_datetime(candle_count=candle_count, target_date=target_date)
...
def get_subscribed_instrument_keys() -> list:
    ...
    if not isinstance(subscribed_keys, (list, tuple, set)):
        return []
    unique_keys = []
    seen_keys = set()
...
def get_contract_info_by_key(instrument_key: str) -> dict:
    ...
    if not normalized_key:
        return {}
    main_key = str(getattr(config, "MAIN_NIFTY_SECURITY", "NSE_INDEX|Nifty 50") or "NSE_INDEX|Nifty 50").strip()
    if normalized_key == main_key:
        return {"instrument_key": normalized_key, "instrument_type": "INDEX", "strike_price": None, "expiry": None,
    ...
    try:
        ...
        logger.warning("Contract lookup failed. instrument_key=%s, error=%s: %s", normalized_key, type(ex).__name__, ex)
        resolved = None
    ...
    if isinstance(cached_contracts, list):
        for item in cached_contracts:
            if not isinstance(item, dict):
                continue
            if item.get("instrument_key") == normalized_key:
                return item
    return {"instrument_key": normalized_key}

def normalize_option_type(instrument_type: Any) -> str | None:
    """
