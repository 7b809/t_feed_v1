...
def get_isolated_instrument_type_from_state(selected_state: dict) -> str | None:
    ...
    if not isinstance(contract_info, dict):
        return None
    instrument_type = contract_info.get("instrument_type") or contract_info.get("option_type")
...
def _format_numeric_value(value: Any, unavailable_text: str = "not_available", decimal_places: int | None = None) -> str:
    if value is None:
...
def _format_price_value(value: Any, unavailable_text: str = "N/A", decimal_places: int | None = None) -> str:
    if value is None:
        return unavailable_text
    if decimal_places is None:
        decimal_places = safe_int(getattr(config, "EMA_ISOLATED_ALERT_PRICE_DECIMAL_PLACES", 2), default=2)
...
def _format_telegram_timestamp(timestamp_value: Any, unavailable_text: str = "N/A") -> str:
    ...
    market_timezone_name = str(getattr(config, "MARKET_TIMEZONE", "Asia/Kolkata") or "Asia/Kolkata").strip()
    try:
...
def extract_ema_candle_details(ema_event: dict) -> dict:
    ...
    if not isinstance(tick, dict):
        tick = {}
    close_price = _safe_float(candle.get("close"), default=_safe_float(ema_event.get("close"), default=_safe_float(ema_event.get("ltp"))))
    ...
    candle_timestamp = candle.get("timestamp") or ema_event.get("timestamp") or tick.get("timestamp")
...
def get_suggested_order_option_type(instruments: list | None, cross_type: str, isolated_instrument_type: str | None) -> str | None:
    ...
    if isinstance(instruments, list):
        for item in instruments:
            if not isinstance(item, dict):
                continue
            option_type = normalize_option_type(item.get("option_type") or item.get("instrument_type"))
            if option_type:
                return option_type
    if "bullish" in normalized_cross_type:
        return normalize_option_type(getattr(config, "EMA_ALERT_BULLISH_OPTION_TYPE", None))
    if "bearish" in normalized_cross_type:
        return normalize_option_type(getattr(config, "EMA_ALERT_BEARISH_OPTION_TYPE", None))
    return None
...
def get_option_chain_instruments_for_ema(*, cross_type: str, isolated_instrument_type: str | None):
    from services.main_index_ltp_service import get_nearest_option_instruments
    suggested_order_option_type = get_suggested_order_option_type(instruments=[], cross_type=cross_type, isolated_instrument_type=isolated_instrument_type)
    if not suggested_order_option_type:
        error_message = "Could not resolve suggested option type for isolated EMA alert."
        logger.warning("%s cross_type=%s, isolated_type=%s", error_message, cross_type, isolated_instrument_type)
        return {
            "status": "failed",
            "success": False,
    ...
    requested_count = max(1, safe_int(getattr(config, "MAIN_INDEX_NEAREST_INSTRUMENTS_COUNT", 3), default=3))
    try:
        option_chain_result = get_nearest_option_instruments(option_type=suggested_order_option_type, count=requested_count)
    except Exception as ex:
        error_message = f"{type(ex).__name__}: {ex}"
        logger.exception("EMA option-chain instrument lookup failed. cross_type=%s, isolated_type=%s, order_side=%s, error=%s", cross_type, isolated_instrument_type, suggested_order_option_type, error_message)
        return {
            "status": "failed",
            "success": False,
    ...
    logger.info("EMA option-chain instruments resolved. success=%s, cross_type=%s, isolated_type=%s, order_side=%s, spot_price=%s, nearest_count=%s, budget_count=%s, expiry=%s",
                success, cross_type, isolated_instrument_type, suggested_order_option_type, option_chain_result.get("underlying_spot_price"),
                len(nearest_instruments), len(budget_instruments), option_chain_result.get("expiry_date"))
    return {
...
def enrich_option_chain_instruments(*, instruments: list, isolated_instrument_key: str) -> list:
    ...
    for item in instruments:
        ...
        option_type = normalize_option_type(instrument.get("option_type") or instrument.get("instrument_type"))
        if option_type:
            instrument["instrument_type"] = option_type
            instrument["option_type"] = option_type
        instrument["is_isolated_instrument"] = bool(instrument_key and normalized_isolated_key and instrument_key == normalized_isolated_key)
...
def format_suggested_order_instruments(instruments: list) -> str:
    ...
    decimal_places = max(0, safe_int(getattr(config, "EMA_ISOLATED_ALERT_PRICE_DECIMAL_PLACES", 2), default=2))
    formatted_instruments = []
    for item in instruments:
        if not isinstance(item, dict):
            continue
        option_label = _format_option_label(item.get("strike_price"), (item.get("option_type") or item.get("instrument_type")))
        market_data = item.get("market_data") or {}
        if not isinstance(market_data, dict):
            market_data = {}
        option_ltp = _safe_float(item.get("ltp"), default=_safe_float(market_data.get("ltp")))
        volume = _safe_float(item.get("volume"), default=_safe_float(market_data.get("volume")))
        ...
        formatted_instruments.append("\n".join([f"- {option_label}{isolated_text}", f"  LTP: {ltp_text}", f"  Volume: {volume_text}"]))
...
# ============================================================
def format_budget_range_instruments(instruments: list, order_option_type: str | None) -> str:
    decimal_places = max(0, safe_int(getattr(config, "EMA_ISOLATED_ALERT_PRICE_DECIMAL_PLACES", 2), default=2))
    minimum_price = _format_price_value(getattr(config, "EMA_ALERT_BUDGET_MIN_PRICE", 50.0), decimal_places=decimal_places)
    maximum_price = _format_price_value(getattr(config, "EMA_ALERT_BUDGET_MAX_PRICE", 120.0), decimal_places=decimal_places)
    ...
    heading = f"Budget Range Option-Chain Instruments ({minimum_price} to {maximum_price}):"
    if not isinstance(instruments, list) or not instruments:
...
def normalize_ema_cross_direction(ema_event: dict) -> str:
    ...
    if "bullish" in cross_type or cross_type in bullish_values or current_signal in bullish_values:
        return "bullish"
    if "bearish" in cross_type or cross_type in bearish_values or current_signal in bearish_values:
        return "bearish"
    return "unknown"
...
def should_skip_isolated_ema_alert_for_minute_direction(instrument_key: str, ema_event: dict, timestamp_value: Any = None) -> tuple[bool, str, str]:
    ...
    skip_alert = state.check_and_reserve_ema_minute_key(alert_key=alert_key, state_date=alert_date)
...
def _send_telegram_message(title: str, message: str, level: str) -> bool:
    try:
        return bool(telegram_service.send_message(title=title, message=message, level=level))
    except Exception as ex:
        logger.error("Telegram delivery failed. title=%s, level=%s, error=%s: %s", title, level, type(ex).__name__, ex)
        return False
...
def _dispatch_algo_app_payload(payload: dict) -> bool:
    ...
    try:
        ...
        logger.error("Algo App dispatch failed. event_id=%s, instrument_key=%s, error=%s: %s", payload.get("event_id"), (payload.get("instrument") or {}).get("instrument_key"), type(ex).__name__, ex)
        return False
...
def build_isolated_ema_alert_payload(
    ...
    instrument_key = str(ema_event.get("instrument_key") or selected_state.get("instrument_key") or contract_info.get("instrument_key") or "").strip()
    strike_price = _safe_float(contract_info.get("strike_price"))
    normalized_isolated_type = normalize_option_type(isolated_instrument_type or contract_info.get("instrument_type") or contract_info.get("option_type"))
    normalized_order_side = normalize_option_type(suggested_order_option_type)
    selected_level = selected_state.get("selected_level") or selected_state.get("level") or "N/A"
    ...
    event_timestamp = normalized_ema_candle.get("timestamp") or _format_market_timestamp(ema_event.get("timestamp"))
    normalized_direction = str(alert_direction or normalize_ema_cross_direction(ema_event) or "unknown").strip().lower()
    event_id = f"EMA-{instrument_key.replace('|', '-')}-{now_market.strftime('%Y%m%dT%H%M%S')}-{normalized_direction}-{uuid4().hex[:8]}"
    nifty_ltp = state.get_latest_main_index_ltp_value()
    isolated_snapshot = state.get_latest_instrument_ltp_snapshot(instrument_key) if instrument_key else {}
    if not isinstance(isolated_snapshot, dict):
        isolated_snapshot = {}
    isolated_instrument_ltp = _safe_float(isolated_snapshot.get("live_ltp"), default=_safe_float(isolated_snapshot.get("ltp"), default=_safe_float(normalized_ema_candle.get("close"))))
    ema_calculation_mode = str(ema_event.get("ema_calculation_mode", get_live_ema_calculation_mode_text()) or get_live_ema_calculation_mode_text()).strip().lower()
    ...
    for item in suggested_instruments:
        ...
        item_option_type = normalize_option_type(instrument.get("instrument_type") or instrument.get("option_type"))
        ...
        item_live_ltp = _safe_float(instrument.get("live_ltp"), default=_safe_float(instrument.get("ltp")))
        if item_live_ltp is None and item_candle:
            item_live_ltp = _safe_float(item_candle.get("close"))
        item_is_isolated = bool(item_key and instrument_key and item_key == instrument_key)
        normalized_instrument = {
            ...
            "lot_size": safe_int(instrument.get("lot_size"), default=0),
            "underlying_key": instrument.get("underlying_key"),
            "underlying_spot_price": _safe_float(instrument.get("underlying_spot_price")),
    ...
    payload = {
        ...
        "event_id": (event_id if bool(getattr(config, "ALGO_APP_INCLUDE_EVENT_ID", True)) else None),
        "event_type": "isolated_instrument_ema_alert",
        "source": getattr(config, "ALGO_APP_SOURCE_NAME", "option_feed_engine"),
        "market": "NSE",
        ...
        "instrument": {
            ...
            "lot_size": safe_int(contract_info.get("lot_size"), default=0),
            "live_ltp": isolated_instrument_ltp,
            "isolated": True,
        },
        "opening_range": {
            "selected_level": selected_level,
            "selected_level_value": _safe_float(selected_state.get("level_value") or selected_state.get("selected_level_value")),
            "trigger_price": _safe_float(selected_state.get("trigger_price")),
            "trigger_field": selected_state.get("trigger_field"),
            "touch_source": selected_state.get("touch_source") or selected_state.get("selection_source"),
            "touch_time": selected_state.get("touch_time") or selected_state.get("selected_at"),
        ...
        "ema": {
            ...
            "price": _safe_float(ema_event.get("close", ema_event.get("ltp"))),
        ...
        "order_suggestion": {
            ...
            "budget_filter": {
                ...
                "minimum_price": _safe_float(getattr(config, "EMA_ALERT_BUDGET_MIN_PRICE", 50.0)),
                "maximum_price": _safe_float(getattr(config, "EMA_ALERT_BUDGET_MAX_PRICE", 120.0)),
                "maximum_instruments": safe_int(getattr(config, "EMA_ALERT_BUDGET_MAX_INSTRUMENTS", 2), default=2),
                "sort_mode": str(getattr(config, "EMA_ALERT_BUDGET_SORT_MODE", "nearest_to_budget_midpoint")).strip().lower(),
    ...
    if not bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_OPENING_RANGE", True)):
        payload.pop("opening_range", None)
    if not bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_EMA_VALUES", True)):
        ...
        payload["ema"].pop("previous_fast_value", None)
        payload["ema"].pop("previous_slow_value", None)
    if not bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_CANDLE", True)):
        payload["ema"].pop("candle", None)
    if not bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_NEAREST_INSTRUMENTS", True)):
        payload["order_suggestion"]["nearest_instruments"] = []
    include_nearest_candles = bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_NEAREST_INSTRUMENT_CANDLES", True))
    ...
    if not bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_BUDGET_INSTRUMENTS", True)):
        payload["order_suggestion"]["budget_filter"]["instruments"] = []
        payload["order_suggestion"]["budget_filter"]["matched_count"] = 0
    include_budget_candles = bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_BUDGET_INSTRUMENT_CANDLES", True))
    if not include_budget_candles:
        for instrument in payload["order_suggestion"]["budget_filter"]["instruments"]:
            if isinstance(instrument, dict):
                instrument.pop("candle", None)
    if not bool(getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_RAW_EMA_EVENT", True)):
...
def build_isolated_ema_telegram_message(payload: dict) -> str:
    ...
    if not isinstance(candle, dict):
        candle = {}
    strike = _format_numeric_value(instrument.get("strike_price"), unavailable_text="N/A")
    option_type = normalize_option_type(instrument.get("instrument_type") or instrument.get("option_type")) or "N/A"
    selected_level = str(opening_range.get("selected_level") or "N/A").strip().upper()
    nifty_ltp = _format_numeric_value(market_snapshot.get("nifty_ltp"), unavailable_text="N/A", decimal_places=decimal_places)
    ...
    if not signal or signal.lower() in {"n/a", "na", "none", "null", "not_available", "unknown"}:
        signal = "N/A"
    else:
        signal = signal.replace("_", " ").title()
    suggested_order_side = normalize_option_type(order_suggestion.get("suggested_order_side")) or "N/A"
    raw_calculation_mode = str(ema_data.get("calculation_mode") or "").strip()
    if raw_calculation_mode:
        calculation_mode = raw_calculation_mode.replace("_", " ").title().replace("Ltp", "LTP")
    else:
        calculation_mode = "N/A"
    candle_close = _format_price_value(candle.get("close"), decimal_places=decimal_places)
    ...
    close_low_movement = _format_numeric_value(close_minus_low, unavailable_text="N/A", decimal_places=decimal_places)
    candle_time = _format_telegram_timestamp(candle.get("timestamp") or ema_data.get("timestamp"))
    instrument_key = str(instrument.get("instrument_key") or "not_available").strip()
    message_sections = [f"{strike} {option_type} | {selected_level} Cross | NIFTY {nifty_ltp}"]
    ema_detail_lines = [
        "EMA Cross Details:",
    ...
    if bool(getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_NEAREST_ORDER_INSTRUMENTS", True)):
    ...
    if bool(getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_BUDGET_INSTRUMENTS", True)):
        ...
        budget_text = format_budget_range_instruments(budget_instruments, suggested_order_side)
        if budget_text:
            message_sections.append(budget_text)
    return "\n\n".join(section.strip() for section in message_sections if isinstance(section, str) and section.strip())
...
def process_selected_or_ema_cross_alert(ema_event: dict) -> bool:
    ...
    algo_enabled = bool(getattr(config, "ALGO_APP_ENABLED", False))
    ...
    if not isinstance(selected_state, dict):
        logger.warning("Isolated EMA alert skipped because selected instrument state is invalid.")
        return False
    ...
    if not isolated_key or not event_key:
        logger.warning("Isolated EMA alert skipped because instrument key is unavailable. isolated_key=%s, event_key=%s", isolated_key or None, event_key or None)
        return False
    ...
    if not isolated_instrument_type:
        isolated_instrument_type = normalize_option_type(contract_info.get("instrument_type") or contract_info.get("option_type"))
    cross_type = str(ema_event.get("cross_type") or "").strip()
    if not cross_type:
        logger.warning("Isolated EMA alert skipped because cross_type is unavailable. instrument_key=%s", event_key)
        return False
    alert_direction = normalize_ema_cross_direction(ema_event)
    if alert_direction not in {"bullish", "bearish"}:
        logger.warning("Isolated EMA alert skipped because direction could not be resolved. instrument_key=%s, cross_type=%s, current_signal=%s", event_key, cross_type, ema_event.get("current_signal"))
        return False
    alert_icon = _get_ema_alert_icon(direction=alert_direction, cross_type=cross_type)
    ...
    if DEFAULT_LIVE_EMA_CALCULATION_MODE:
        (skip_alert, minute_alert_key, alert_direction) = should_skip_isolated_ema_alert_for_minute_direction(instrument_key=event_key, ema_event=ema_event, timestamp_value=event_timestamp)
        if skip_alert:
            logger.info("Skipping duplicate isolated EMA alert. instrument_key=%s, key=%s, direction=%s", event_key, minute_alert_key, alert_direction)
            return False
    ...
    try:
        ...
        suggested_order_option_type = get_suggested_order_option_type(instruments=[], cross_type=cross_type, isolated_instrument_type=isolated_instrument_type)
        if not suggested_order_option_type:
            logger.error("Isolated EMA order side could not be resolved. instrument_key=%s, cross_type=%s, isolated_type=%s", event_key, cross_type, isolated_instrument_type)
            if duplicate_key_reserved and minute_alert_key:
                state.release_ema_minute_key(minute_alert_key)
            return False
        option_chain_selection = get_option_chain_instruments_for_ema(cross_type=cross_type, isolated_instrument_type=isolated_instrument_type)
        if not isinstance(option_chain_selection, dict):
            option_chain_selection = {"status": "failed", "success": False, "nearest_instruments": [], "nearest_strikes": [], "budget_instruments": [], "budget_range": {}, "error": "Invalid option-chain selection result."}
        suggested_instruments = option_chain_selection.get("nearest_instruments", [])
        budget_instruments = option_chain_selection.get("budget_instruments", [])
        if not isinstance(suggested_instruments, list):
            suggested_instruments = []
        if not isinstance(budget_instruments, list):
            budget_instruments = []
        enriched_nearest_instruments = enrich_option_chain_instruments(instruments=suggested_instruments, isolated_instrument_key=event_key)
        enriched_budget_instruments = enrich_option_chain_instruments(instruments=budget_instruments, isolated_instrument_key=event_key)
        option_chain_spot_price = _safe_float(option_chain_selection.get("underlying_spot_price"))
        ...
        if not isinstance(payload, dict):
            logger.error("Isolated EMA payload builder returned an invalid result. instrument_key=%s", event_key)
            if duplicate_key_reserved and minute_alert_key:
                state.release_ema_minute_key(minute_alert_key)
            return False
        ...
        order_suggestion["nearest_strikes"] = deepcopy(option_chain_selection.get("nearest_strikes", []))
        order_suggestion["nearest_instruments"] = deepcopy(enriched_nearest_instruments)
        budget_filter_payload = order_suggestion.get("budget_filter", {})
        if not isinstance(budget_filter_payload, dict):
            budget_filter_payload = {}
        option_chain_budget = option_chain_selection.get("budget_range", {})
        if not isinstance(option_chain_budget, dict):
            option_chain_budget = {}
        budget_filter_payload.update({
            "enabled": option_chain_budget.get("enabled", bool(getattr(config, "EMA_ALERT_BUDGET_RANGE_ENABLED", True))),
            "minimum_price": option_chain_budget.get("minimum_price", getattr(config, "EMA_ALERT_BUDGET_MIN_PRICE", 50.0)),
            "maximum_price": option_chain_budget.get("maximum_price", getattr(config, "EMA_ALERT_BUDGET_MAX_PRICE", 120.0)),
            "maximum_instruments": option_chain_budget.get("maximum_instruments", getattr(config, "EMA_ALERT_BUDGET_MAX_INSTRUMENTS", 2)),
            "sort_mode": option_chain_budget.get("sort_mode", getattr(config, "EMA_ALERT_BUDGET_SORT_MODE", "nearest_to_budget_midpoint")),
            "range_inclusive": option_chain_budget.get("range_inclusive", True),
        ...
        market_snapshot["option_chain_expiry"] = option_chain_selection.get("expiry_date")
        market_snapshot["option_data_source"] = option_chain_selection.get("data_source")
        payload["market_snapshot"] = market_snapshot
        telegram_message = build_isolated_ema_telegram_message(payload)
        logger.info("Processing isolated EMA alert. event_id=%s, instrument_key=%s, cross_type=%s, direction=%s, isolated_type=%s, order_side=%s, option_chain_success=%s, nearest_instruments=%s, budget_matches=%s",
                    payload.get("event_id"), event_key, cross_type, alert_direction, isolated_instrument_type, suggested_order_option_type,
                    option_chain_selection.get("success"), len(enriched_nearest_instruments), len(enriched_budget_instruments))
        telegram_sent = False
        algo_dispatched = False
        if telegram_enabled:
            telegram_sent = _send_telegram_message(title=telegram_title, message=telegram_message, level="EMA")
        if algo_enabled:
        ...
        if not delivery_accepted and duplicate_key_reserved and minute_alert_key:
            state.release_ema_minute_key(minute_alert_key)
        delivery_record = {
            "telegram": {"enabled": telegram_enabled, "attempted": telegram_enabled, "success": telegram_sent, "title": telegram_title},
            "algo_app": {
                "enabled": algo_enabled,
                "attempted": algo_enabled,
                "dispatched": algo_dispatched,
                "delivery_mode": "background" if bool(getattr(config, "ALGO_APP_SEND_IN_BACKGROUND", True)) else "synchronous",
            },
        }
        alert_record = {
        ...
        if delivery_accepted:
            ...
            logger.info("Isolated EMA alert accepted. event_id=%s, instrument_key=%s, telegram_sent=%s, algo_dispatched=%s", payload.get("event_id"), event_key, telegram_sent, algo_dispatched)
        else:
            logger.warning("Isolated EMA alert was not accepted by Telegram or Algo App. instrument_key=%s, event_id=%s", event_key, payload.get("event_id"))
        ...
        if duplicate_key_reserved and minute_alert_key:
            try:
                ...
                logger.error("Failed releasing isolated EMA duplicate key after processing failure. key=%s, error=%s: %s", minute_alert_key, type(release_ex).__name__, release_ex)
        logger.exception("Isolated EMA alert processing failed. instrument_key=%s, cross_type=%s, error=%s: %s", event_key, cross_type, type(ex).__name__, ex)
        return False
...
def _build_empty_opening_range_ema_payload(latest_main_index_ltp: float | None = None) -> dict:
    return {
        "opening_range": {},
...
def get_opening_range_levels_for_ema_event(instrument_key: str) -> dict:
    ...
    if not DEFAULT_EMA_CROSS_INCLUDE_OPENING_RANGE_LEVELS:
        return _build_empty_opening_range_ema_payload(latest_main_index_ltp=latest_main_index_ltp)
    if instrument_key is None:
        return _build_empty_opening_range_ema_payload(latest_main_index_ltp=latest_main_index_ltp)
