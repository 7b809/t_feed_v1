...
CATEGORY_LABELS = {
    "opening_range_isolation": "Opening Range Isolation",
    "opening_range_touch": "Opening Range Touch",
    "ema": "EMA",
    "ema_budget": "EMA Budget",
    "ema_order": "EMA Order",
}

CATEGORY_SHORT_CODES = {
    "opening_range_isolation": "ori",
    "opening_range_touch": "ort",
    "ema": "ema",
    "ema_budget": "emb",
    "ema_order": "emo",
}
...
class TelegramTokenBot:
    def __init__(self):
        ...
        self.runtime_config_enabled = bool(
            getattr(config, "TELEGRAM_RUNTIME_CONFIG_ENABLED", True)
        )
        self.config_confirmation_required = bool(
            getattr(config, "TELEGRAM_RUNTIME_CONFIG_CONFIRMATION_REQUIRED", True)
        )
        self.custom_value_enabled = bool(
            getattr(config, "TELEGRAM_RUNTIME_CONFIG_CUSTOM_VALUE_ENABLED", True)
        )
        self.config_session_timeout = max(
            30, int(getattr(config, "TELEGRAM_RUNTIME_CONFIG_TIMEOUT_SECONDS", 120))
        )
        self.api_url = (
            f"https://api.telegram.org/bot{self.bot_token}" if self.bot_token else None
        )
        self.poll_seconds = max(
            1, int(getattr(config, "TELEGRAM_TOKEN_BOT_POLL_SECONDS", 3))
        )
        self.long_poll_timeout = max(
            1, int(getattr(config, "TELEGRAM_TOKEN_BOT_LONG_POLL_TIMEOUT", 20))
        )
        ...
        # --------------------------------------------------------
        # Manual instrument isolation state
        #
        # One isolation flow per Telegram chat.
        #
        # Example:
        #
        # {
        ...
        # }
        # --------------------------------------------------------
        self._pending_isolation_inputs = {}
    ...
    def _request(
        ...
        try:
            response = requests.post(
                url, json=payload or {}, timeout=timeout or self.long_poll_timeout + 10
            )
            try:
        ...
        try:
            return bool(
                telegram_service.send_message(
                    title="Token Bot", message=text, level=level
                )
            )
        except Exception as ex:
            logger.error(
                "Telegram service send failed. error=%s: %s", type(ex).__name__, ex
            )
            return False
    ...
    def _send_bot_message(
        ...
        if not result.get("ok"):
            logger.warning(
                "Telegram sendMessage failed. chat_id=%s, result=%s", chat_id, result
            )
        return result

    def _edit_bot_message(
        ...
        if not result.get("ok"):
            ...
            if "message is not modified" not in description:
                logger.warning(
                    "Telegram editMessageText failed. chat_id=%s, message_id=%s, result=%s",
                    chat_id,
                    message_id,
                    result,
                )
    ...
    def _answer_callback_query(
        self, callback_query_id, text: str | None = None, show_alert: bool = False
    ):
        payload = {
    ...
    @staticmethod
    def _normalize_option_type(option_type: str) -> str | None:
        value = str(option_type or "").strip().upper()
        if value in {"CE", "CALL", "C"}:
            return "CE"
        if value in {"PE", "PUT", "P"}:
            return "PE"
        return None

    @staticmethod
    def _format_strike_price(value) -> str:
        try:
            strike = float(value)
    ...
    def _cleanup_expired_config_sessions(self):
        ...
        with self._state_lock:
            expired_inputs = [
                chat_id
                for chat_id, state in self._pending_config_inputs.items()
                if now - float(state.get("created_at", 0)) > self.config_session_timeout
            ]
            ...
            expired_confirmations = [
                key
                for key, state in self._pending_config_confirmations.items()
                if now - float(state.get("created_at", 0)) > self.config_session_timeout
            ]
            ...
            # ----------------------------------------------------
            # Clean up expired isolation flows too.
            # ----------------------------------------------------
            expired_isolations = [
                chat_id
                for chat_id, state in self._pending_isolation_inputs.items()
                if now - float(state.get("created_at", 0)) > self.config_session_timeout
            ]
            for chat_id in expired_isolations:
                self._pending_isolation_inputs.pop(chat_id, None)

    def delete_message(self, chat_id, message_id) -> bool:
        result = self._request(
            "deleteMessage", {"chat_id": chat_id, "message_id": message_id}, timeout=10
        )
        if not result.get("ok"):
            logger.warning(
                "Telegram deleteMessage failed. chat_id=%s, message_id=%s, result=%s",
                chat_id,
                message_id,
                result,
            )
    ...
    def set_commands(self) -> bool:
        ...
        commands = [
            ...
            {"command": "isolate", "description": "Manually isolate option instrument"},
        ]
            ...
            commands.extend(
                [
                    {"command": "config", "description": "Open runtime configuration"},
                    {
                        ...
                        "description": "Show runtime config status",
                    },
                    {
                        "command": "config_refresh",
                        "description": "Reload runtime config from MongoDB",
                    },
                ]
            )
    ...
    def get_updates(self) -> list:
        ...
        result = self._request(
            "getUpdates", payload, timeout=self.long_poll_timeout + 10
        )
        if not result.get("ok"):
    ...
    def _handle_token_status_command(self):
        ...
        self._send_message(
            "Current token document status:\n\n" + self._format_json_for_telegram(info),
            level="TOKEN",
        )
    ...
    def _handle_token_status_validate_command(self):
        ...
        self._send_message(
            ...
            + self._format_json_for_telegram(safe_result),
            level="TOKEN",
        )
    ...
    def _handle_profile_command(self):
        ...
        self._send_message(
            ...
            + self._format_json_for_telegram(profile_result),
            level="TOKEN",
        )
    ...
    def _handle_funds_command(self):
        ...
        self._send_message(
            ...
            + self._format_json_for_telegram(funds_result),
            level="TOKEN",
        )

    # ============================================================
    # MANUAL INSTRUMENT ISOLATION
    # ============================================================

    def _isolate_cancel_keyboard(self) -> dict:
        return {
            "inline_keyboard": [[{"text": "Cancel", "callback_data": "iso:cancel"}]]
        }

    def _isolate_option_keyboard(self) -> dict:
        return {
            "inline_keyboard": [
                [
                    {"text": "CE", "callback_data": "iso:type:CE"},
                    {"text": "PE", "callback_data": "iso:type:PE"},
                ],
                [{"text": "Cancel", "callback_data": "iso:cancel"}],
            ]
        }

    def _isolate_confirmation_keyboard(self) -> dict:
        return {
            "inline_keyboard": [
                [
                    {"text": "Confirm", "callback_data": "iso:confirm"},
                    {"text": "Cancel", "callback_data": "iso:cancel"},
                ]
            ]
        }

    def _clear_isolation_state(self, chat_id):
        with self._state_lock:
            self._pending_isolation_inputs.pop(str(chat_id), None)

    def _start_isolation_flow(self, chat_id, requested_by):
        with self._state_lock:
            self._pending_isolation_inputs[str(chat_id)] = {
                "step": "strike",
                "strike_price": None,
                "option_type": None,
        ...
        self._send_bot_message(
            ...
            reply_markup=self._isolate_cancel_keyboard(),
        )

    def _handle_isolate_command(self, message: dict):
        """
        Supports both:
        ...
        /isolate without arguments starts an interactive
        step-by-step flow.

        /isolate with strike + option type skips directly
        to confirmation.
        """
        ...
        requested_by = self._config_actor(message)
        ...
        if len(parts) == 1:
            self._start_isolation_flow(chat_id=chat_id, requested_by=requested_by)
            return
        ...
        if len(parts) != 3:
            self._send_bot_message(
                chat_id,
                "Invalid /isolate command.\n\nUse either:\n\n/isolate\nfor interactive mode.\n\nor:\n\n/isolate <strike> <CE|PE>\n\nExamples:\n/isolate 24200 CE\n/isolate 24200 PE",
                reply_markup=self._isolate_cancel_keyboard(),
            )
            return

        raw_strike = str(parts[1] or "").strip()
        raw_option_type = str(parts[2] or "").strip()
        ...
        try:
            ...
            self._send_bot_message(
                chat_id,
                "Invalid strike price.\n\nPlease provide a valid positive strike price.\n\nExample:\n/isolate 24200 CE",
                reply_markup=self._isolate_cancel_keyboard(),
            )
            return
        ...
        normalized_option_type = self._normalize_option_type(raw_option_type)
        ...
        # --------------------------------------------------------
        # Store state and show confirmation
        # --------------------------------------------------------
        ...
        self._send_isolation_confirmation(chat_id)

    def _handle_isolation_strike_message(self, message: dict, pending_state: dict):
        chat = message.get("chat") or {}
        chat_id = chat.get("id")
        text = str(message.get("text") or "").strip()
        if not chat_id:
        ...
        # --------------------------------------------------------
        # Allow /cancel during strike entry.
        # --------------------------------------------------------

        if text.lower() == "/cancel":
            self._clear_isolation_state(chat_id)
            self._send_bot_message(chat_id, "Manual instrument isolation cancelled.")
            return

        # --------------------------------------------------------
        # Ignore another command except /isolate.
        # --------------------------------------------------------

        if text.startswith("/"):
            command = self._normalize_command(text)
            if command == "/isolate":
                self._handle_isolate_command(message)
                return
            self._send_bot_message(
                chat_id,
                "Please enter the strike price or press Cancel.",
                reply_markup=self._isolate_cancel_keyboard(),
            )
            return
        ...
        # --------------------------------------------------------
        # Save strike and move to option type.
        # --------------------------------------------------------

        with self._state_lock:
            state = self._pending_isolation_inputs.get(str(chat_id))
            if not state:
                return
            state["step"] = "option_type"
            state["strike_price"] = strike_price
            state["option_type"] = None
            state["created_at"] = time.time()

        self._send_bot_message(
            chat_id,
            f"Manual Instrument Isolation\n\nStrike: {self._format_strike_price(strike_price)}\n\nPlease select option type:",
            reply_markup=self._isolate_option_keyboard(),
        )

    def _send_isolation_confirmation(self, chat_id):
        with self._state_lock:
            state = deepcopy_value(self._pending_isolation_inputs.get(str(chat_id)))
        if not state:
            self._send_bot_message(
                chat_id, "Isolation session expired. Please use /isolate again."
            )
            return
        ...
        self._send_bot_message(
            chat_id,
            f"Confirm Manual Instrument Isolation\n\nStrike: {self._format_strike_price(strike_price)}\nOption Type: {option_type}\n\nDo you want to isolate this instrument?",
            reply_markup=self._isolate_confirmation_keyboard(),
        )

    def _handle_isolation_option_selection(self, chat_id, option_type):
        normalized_option_type = self._normalize_option_type(option_type)
        if not normalized_option_type:
            self._send_bot_message(
                chat_id,
                "Invalid option type selection.",
                reply_markup=self._isolate_option_keyboard(),
            )
            return
        ...
        with self._state_lock:
            ...
            if not state:
                ...
                return
            if state.get("step") != "option_type":
                self._send_bot_message(
                    chat_id,
                    "This isolation action is no longer at the option selection step.",
                )
                return
            state["option_type"] = normalized_option_type
            state["step"] = "confirmation"
            state["created_at"] = time.time()

        self._send_isolation_confirmation(chat_id)

    def _handle_isolation_confirmation(self, chat_id):
        with self._state_lock:
            state = deepcopy_value(self._pending_isolation_inputs.get(str(chat_id)))
        if not state:
            self._send_bot_message(
        ...
        if state.get("step") != "confirmation":
            self._send_bot_message(
                chat_id, "Isolation confirmation is not available at the current step."
            )
            return

        strike_price = state.get("strike_price")
        option_type = self._normalize_option_type(state.get("option_type"))
        requested_by = str(state.get("requested_by") or "telegram_user")

        if strike_price is None:
            self._send_bot_message(
                chat_id, "Strike price is missing. Please use /isolate again."
            )
            self._clear_isolation_state(chat_id)
            return
        ...
        if not option_type:
            self._send_bot_message(
                chat_id, "Option type is missing. Please use /isolate again."
            )
            self._clear_isolation_state(chat_id)
            return

        # --------------------------------------------------------
        # Clear state BEFORE starting the worker.
        #
        # This prevents a second confirm callback from executing
        # the same isolation twice.
        # --------------------------------------------------------

        self._clear_isolation_state(chat_id)
        self._send_bot_message(
            chat_id,
            f"Manual instrument isolation started.\n\nStrike: {self._format_strike_price(strike_price)}\nOption Type: {option_type}\nRequested By: {requested_by}\nSource: telegram\n\nProcessing in background...",
        )

        worker = Thread(
            target=self._run_manual_instrument_isolation,
            kwargs={
                "chat_id": chat_id,
                "strike_price": strike_price,
                "normalized_option_type": option_type,
                "requested_by": requested_by,
            },
            name="telegram-manual-instrument-isolation",
            daemon=True,
        )
        worker.start()
        logger.info(
            "Manual instrument isolation worker started from Telegram. chat_id=%s, strike=%s, option_type=%s, requested_by=%s",
            chat_id,
            strike_price,
            option_type,
            requested_by,
    ...
    def _run_manual_instrument_isolation(
        self, chat_id, strike_price, normalized_option_type, requested_by
    ):
        """
        Execute the exact same service used by the API.

        IMPORTANT:
        No HTTP request is made to the FastAPI endpoint.

        Both API and Telegram directly call:
        ...
        try:
            logger.warning(
                "Manual instrument isolation Telegram request received. strike=%s, option_type=%s, requested_by=%s",
                strike_price,
                normalized_option_type,
                requested_by,
            )
            result = manual_instrument_isolation_service.isolate(
                strike_price=strike_price,
                option_type=normalized_option_type,
                requested_by=requested_by,
                source="telegram",
            )
            self._send_manual_isolation_result(
                chat_id=chat_id,
                result=result,
                strike_price=strike_price,
                option_type=normalized_option_type,
            ...
            logger.exception(
                "Manual instrument isolation from Telegram failed. strike=%s, option_type=%s, requested_by=%s",
                strike_price,
                normalized_option_type,
                requested_by,
            )
    ...
    def _send_manual_isolation_result(
        ...
        if success:
            ...
            selected_level = isolated.get("selected_level") or "Not available"

            touch_source = isolated.get("touch_source") or "Not available"

            selected_at = isolated.get("selected_at") or "Not available"
            ...
            previous_instrument_key = previous.get(
                "instrument_key"
            ) or previous_contract.get("instrument_key")

            previous_trading_symbol = previous_contract.get(
                "trading_symbol"
            ...
            if (
                ...
                if previous_instrument_key:
                    message_lines.append(f"• Key: {previous_instrument_key}")

                if previous_trading_symbol:
                    message_lines.append(f"• Symbol: {previous_trading_symbol}")

                if previous_level:
                    message_lines.append(f"• Level: {previous_level}")

                if previous_touch_time:
                    message_lines.append(f"• Touch: {previous_touch_time}")

                if previous_touch_source:
                    message_lines.append(f"• Touch Source: {previous_touch_source}")
        ...
        error_message = str(
            result.get("message") or "Manual instrument isolation was rejected."
        ).strip()
    ...
    def _config_main_keyboard(self) -> dict:
        ...
        rows.append(
            [
                ...
                {"text": "Status", "callback_data": "cfg:status"},
            ]
        )
    ...
    def _handle_config_command(self, chat_id):
        if not self.runtime_config_enabled:
            self._send_bot_message(
                chat_id, "Runtime configuration from Telegram is disabled."
            )
            return
        ...
        try:
            ...
            logger.error(
                "Runtime config initialization failed. error=%s: %s",
                type(ex).__name__,
                ex,
            )
            self._send_bot_message(
                chat_id, "Runtime configuration could not be initialized."
            )
            return
        ...
        self._send_bot_message(
            ...
            reply_markup=self._config_main_keyboard(),
        )
    ...
    def _show_config_category(self, chat_id, message_id, category_code):
        ...
        try:
            registry = get_runtime_config_registry(
                telegram_only=True, category=category
            )
        ...
        for name, metadata in registry.items():
            ...
            button_text = (
                f"{metadata.get('label', name)}: {value_text}{override_marker}"
            )
        ...
        rows.append(
            [
                {"text": "Back", "callback_data": "cfg:home"},
        ...
        self._edit_bot_message(
            chat_id, message_id, text, reply_markup={"inline_keyboard": rows}
        )
    ...
    def _show_config_item(self, chat_id, message_id, config_code):
        ...
        if not name:
            self._send_bot_message(
                chat_id, "Configuration mapping expired. Open /config again."
            )
            return
        ...
        text = (
            f"{metadata.get('label', name)}\n\n"
            f"Current: {current_value}\n"
            f"Default: {default_value}\n"
            f"Source: {source_text}\n"
            f"Type: {metadata.get('value_type')}\n"
            "Restart required: "
            f"{'Yes' if metadata.get('restart_required') else 'No'}\n"
            "Recalculation required: "
            f"{'Yes' if metadata.get('recalculation_required') else 'No'}"
        )
        ...
        for index, suggested_value in enumerate(suggested_values):
            ...
            value_buttons.append(
                {
                    "text": display_value,
                    "callback_data": f"cfg:val:{config_code}:{index}",
                }
            )
        ...
        if self.custom_value_enabled and value_type in {
            "int",
            "float",
            "string",
            "string_list",
            ...
            rows.append(
                [{"text": "Custom Value", "callback_data": f"cfg:custom:{config_code}"}]
            )
                ...
                [
                    {
                        ...
                        "callback_data": f"cfg:reset:{config_code}",
                    }
                ]
            )
        ...
        rows.append(
            [
                {"text": "Back", "callback_data": f"cfg:cat:{category_code}"},
                {"text": "Close", "callback_data": "cfg:close"},
            ]
        )
        self._edit_bot_message(
    ...
    def _prepare_config_value(
        self, chat_id, message_id, callback_query, config_code, raw_value
    ):
        name = self._config_code_to_name.get(config_code)
        if not name:
            self._send_bot_message(
        ...
        confirmation = {
            "name": name,
            "value": deepcopy_value(validated_value),
            "actor": actor,
            "chat_id": str(chat_id),
            "created_at": time.time(),
        }
            ...
            reply_markup={
                "inline_keyboard": [
                    [
                        {
                            "text": "Confirm",
                            "callback_data": f"cfg:confirm:{config_code}",
                        },
                        {"text": "Cancel", "callback_data": f"cfg:item:{config_code}"},
                    ]
                ]
            },
        )
    ...
    def _apply_config_change(self, chat_id, message_id, config_code, confirmation):
        ...
        try:
            result = set_runtime_config(
                ...
                source="telegram",
                chat_id=chat_id,
                reason="Changed from Telegram runtime configuration menu.",
            )
            ...
            self._send_bot_message(
                chat_id, f"Configuration update failed due to {type(ex).__name__}."
            )
            return
        ...
        text = (
            ...
            f"Configuration: {result.get('label', name)}\n"
            f"Old value: {old_value}\n"
            f"New value: {new_value}\n"
            "Changed: "
            f"{'Yes' if result.get('changed') else 'No'}\n"
            "Saved to MongoDB: "
            f"{'Yes' if result.get('persisted') else 'No'}\n"
            "Restart required: "
            f"{'Yes' if result.get('restart_required') else 'No'}\n"
            "Recalculation required: "
            f"{'Yes' if result.get('recalculation_required') else 'No'}"
        )
        self._edit_bot_message(
            chat_id,
            message_id,
            ...
            reply_markup={
                "inline_keyboard": [
                    [
                        {
                            "text": "View Setting",
                            "callback_data": f"cfg:item:{config_code}",
                        },
                        {"text": "Config Home", "callback_data": "cfg:home"},
                    ]
                ]
            },
        )

    def _handle_custom_config_request(
        self, chat_id, message_id, config_code, callback_query
    ):
        name = self._config_code_to_name.get(config_code)
        if not name:
            self._send_bot_message(
        ...
        with self._state_lock:
            self._pending_config_inputs[str(chat_id)] = {
                "name": name,
                "config_code": config_code,
                "actor": self._config_actor(callback_query),
                "created_at": time.time(),
        ...
        guidance = {
            ...
            "float": "Send a numeric value.",
            "string": "Send the new text value.",
            "string_list": "Send comma-separated values.",
            "int_list": "Send comma-separated whole numbers.",
        }.get(value_type, "Send the new value.")
    ...
    def _handle_custom_config_message(self, message: dict, pending_state: dict):
        ...
        confirmation = {
            "name": name,
            "value": deepcopy_value(validated_value),
            "actor": pending_state.get("actor", self._config_actor(message)),
            "chat_id": str(chat_id),
            "created_at": time.time(),
        }
        ...
        if not self.config_confirmation_required:
            try:
                result = set_runtime_config(
                    ...
                    changed_by=confirmation["actor"],
                    source="telegram",
                    chat_id=chat_id,
                    reason="Custom value changed from Telegram.",
                )
            except RuntimeConfigError as ex:
                self._send_bot_message(chat_id, f"Configuration update failed: {ex}")
                return
            ...
            self._send_bot_message(
                chat_id,
                "Configuration Updated\n\nConfiguration: {}\nOld value: {}\nNew value: {}".format(
                    result.get("label", name),
                    self._format_config_value(result.get("old_value")),
                    self._format_config_value(result.get("new_value")),
                ),
                reply_markup={
                    "inline_keyboard": [
                        [{"text": "Open Config", "callback_data": "cfg:home"}]
                    ]
                },
            )
            return
        ...
        self._send_bot_message(
            ...
            "Confirm Configuration Change\n\nConfiguration: {}\nOld value: {}\nNew value: {}".format(
                metadata.get("label", name),
                self._format_config_value(metadata.get("effective_value")),
                self._format_config_value(validated_value),
            ),
            reply_markup={
                "inline_keyboard": [
                    [
    ...
    def _handle_config_reset(self, chat_id, message_id, config_code, callback_query):
        ...
        try:
            result = reset_runtime_config(
                name=name,
                changed_by=self._config_actor(callback_query),
                source="telegram",
                chat_id=chat_id,
                reason="Reset from Telegram runtime configuration menu.",
            )
        except RuntimeConfigError as ex:
        ...
        self._edit_bot_message(
            ...
            "Configuration Reset\n\nConfiguration: {}\nPrevious value: {}\nCurrent value: {}".format(
                result.get("label", name),
                self._format_config_value(result.get("old_value")),
                self._format_config_value(result.get("effective_value")),
            ),
            reply_markup={
                "inline_keyboard": [
                    [
    ...
    def _handle_config_status(self, chat_id, message_id=None):
        ...
        text = (
            ...
            "Enabled: "
            f"{'Yes' if status.get('enabled') else 'No'}\n"
            "Initialized: "
            f"{'Yes' if status.get('initialized') else 'No'}\n"
            "MongoDB connected: "
            f"{'Yes' if status.get('database_connected') else 'No'}\n"
            "Cache enabled: "
            f"{'Yes' if status.get('cache_enabled') else 'No'}\n"
            f"Runtime overrides: {status.get('override_count', 0)}\n"
            f"Registered settings: {status.get('registry_count', 0)}"
        )
        if status.get("error"):
    ...
    def _handle_config_refresh(self, chat_id, message_id=None):
        ...
        keyboard = {
            "inline_keyboard": [[{"text": "Config Home", "callback_data": "cfg:home"}]]
        }
        if message_id:
            self._edit_bot_message(chat_id, message_id, text, reply_markup=keyboard)
        else:
    ...
    def _handle_help_command(self):
        text = (
            ...
            "/isolate\n"
            "Manually isolate an option instrument using interactive mode.\n\n"
            "Direct usage:\n"
            "/isolate 24200 CE\n"
            "/isolate 24200 PE\n\n"
            "Option types:\n"
            "CE / PE\n"
            "CALL / PUT are also accepted."
        )
        if self.runtime_config_enabled:
    ...
    def _handle_raw_token_message(self, chat_id, message_id, text: str):
        ...
        if not raw_token:
            self._send_message(
                "Token message was empty. Use /save_token and try again.",
                level="WARNING",
            )
            return
        ...
        if len(raw_token) < 50:
            self._send_message(
                "Token looks too short and was not saved.\n\nUse /save_token and send the complete Upstox access token.",
                level="WARNING",
            )
            return
        ...
        if not is_valid:
            self._send_message(
                f"The token was not saved because Upstox profile validation failed.\n\nError: {error}\n\nToken message deleted: {deleted}",
                level="ERROR",
            )
            return
        ...
        self._send_message(
            "New Upstox access token saved successfully.", level="SUCCESS"
        )
    ...
    def handle_message(self, message: dict):
        ...
        # --------------------------------------------------------
        # /cancel must be checked BEFORE pending flows.
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # Explicit /isolate always starts/restarts isolation flow.
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # Check pending isolation state.
        #
        # This must happen before pending config state so that
        # the user's strike entry is consumed by the isolation
        # flow and not by runtime configuration.
        # --------------------------------------------------------

        with self._state_lock:
            pending_isolation = deepcopy_value(
                self._pending_isolation_inputs.get(str(chat_id))
            )

        if pending_isolation:
            isolation_step = pending_isolation.get("step")
            if isolation_step == "strike":
                self._handle_isolation_strike_message(message, pending_isolation)
                return

            # Option selection and confirmation are handled
            # through callback buttons.
            #
            # If the user types something while waiting for
            # a button, provide guidance without clearing state.
            if isolation_step == "option_type":
                self._send_bot_message(
                    chat_id,
                    "Please select CE or PE using the buttons below.\n\nYou can also press Cancel.",
                    reply_markup=self._isolate_option_keyboard(),
                )
                return

            if isolation_step == "confirmation":
                self._send_bot_message(
                    chat_id,
                    "Please use Confirm or Cancel to continue.",
                    reply_markup=self._isolate_confirmation_keyboard(),
                )
                return
        ...
        with self._state_lock:
            ...
            pending_config = deepcopy_value(
                self._pending_config_inputs.get(str(chat_id))
            )
    ...
    def handle_callback_query(self, callback_query: dict):
        ...
        if not chat_id or not message_id:
            self._answer_callback_query(
                callback_query_id, "Invalid callback message.", show_alert=True
            )
            return
        ...
        if not self._is_authorized_chat(chat_id):
            ...
            self._answer_callback_query(
                callback_query_id, "Unauthorized.", show_alert=True
            )
            return

        # --------------------------------------------------------
        # Manual isolation callbacks.
        #
        # IMPORTANT:
        # These must be processed BEFORE the runtime-config
        # enabled check.
        #
        # Otherwise /isolate buttons would stop working whenever
        # runtime configuration is disabled.
        # --------------------------------------------------------

        if data.startswith("iso:"):
            ...
            if action == "cancel":
                ...
                self._edit_bot_message(
                    chat_id,
                    message_id,
                    "Manual instrument isolation cancelled.",
            ...
            if action == "type" and len(parts) >= 3:
                self._handle_isolation_option_selection(
                    chat_id=chat_id, option_type=parts[2]
                )
                return
            ...
            if action == "confirm":
                # First edit the current message so the user
                # sees that the confirmation has been consumed.
                self._edit_bot_message(
                    chat_id,
                    message_id,
                    "Manual instrument isolation confirmed.\n\nStarting background processing...",
                    reply_markup={"inline_keyboard": []},
                )
                self._handle_isolation_confirmation(chat_id)
                return

            self._send_bot_message(chat_id, "Unsupported isolation action.")
            return
        ...
        if not self.runtime_config_enabled:
            self._answer_callback_query(
                callback_query_id, "Runtime configuration is disabled.", show_alert=True
            )
            return
        ...
        if data == "cfg:close":
            ...
            self._edit_bot_message(
                chat_id,
                message_id,
                "Runtime configuration menu closed.",
                reply_markup={"inline_keyboard": []},
            )
            return
        ...
        if action == "custom" and len(parts) >= 3:
            self._handle_custom_config_request(
                chat_id=chat_id,
                message_id=message_id,
                config_code=parts[2],
                callback_query=callback_query,
        ...
        if action == "confirm" and len(parts) >= 3:
            ...
            confirmation_key = f"{chat_id}:{config_code}"
            with self._state_lock:
                confirmation = deepcopy_value(
                    self._pending_config_confirmations.pop(confirmation_key, None)
                )

            if not confirmation:
                self._send_bot_message(
                    chat_id, "Confirmation expired. Open /config and try again."
                )
                return
    ...
    def run_loop(self):
        ...
        if self.runtime_config_enabled:
            try:
                ...
                logger.exception(
                    "Runtime configuration startup initialization failed: %s", ex
                )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                logger.exception(
                    "Telegram bot loop error: %s: %s", type(ex).__name__, ex
                )
    ...
    def start(self) -> bool:
        if not self.enabled:
            logger.info(
                "Telegram token bot is disabled or not configured. Check TELEGRAM_ENABLED, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, and TELEGRAM_TOKEN_BOT_ENABLED."
            )
            return False
        ...
        self._thread = Thread(
            target=self.run_loop, name="telegram-token-bot", daemon=True
        )
        self._thread.start()
