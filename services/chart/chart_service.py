...
def fetch_historical_candles_range(
    ...
    logger.info("Fetching historical candles: %s -> %s | %s | %s", from_date, to_date, interval, unit)
    response = api.get_historical_candle_data1(instrument_key, interval, unit, str(to_date), str(from_date))
...
def fetch_historical_candles(
    ...
    while current_end >= start_date:
        current_start = max(start_date, current_end - timedelta(days=batch_days) + timedelta(days=1))
        try:
            ...
            if candles:
                ...
                logger.info("Historical batch loaded for %s: %s -> %s | candles=%s", instrument_key, current_start, current_end, len(candles))
            else:
                logger.info("No historical candles for %s: %s -> %s", instrument_key, current_start, current_end)
        except Exception as exc:
            logger.exception("Historical batch fetch failed for %s: %s -> %s: %s", instrument_key, current_start, current_end, exc)
...
def convert_to_chart_data(candles):
    """
    Convert raw Upstox candles into a frontend-friendly chart representation.
    """
    chart_data = []
...
def get_chart_data(
    ...
    try:
        ...
        logger.exception("Historical candle fetch failed for %s: %s", instrument_key, exc)
    try:
        ...
        logger.exception("Intraday candle fetch failed for %s: %s", instrument_key, exc)
