...
from .candle_utils import (
    get_contract_info_by_key, get_now_market_time, is_option_contract,
    parse_candle_timestamp, safe_float, safe_int, select_post_opening_range_candles,
    serialize_candle
)
from .constants import (
    DEFAULT_BACKFILL_SCAN_ENABLED, DEFAULT_BACKFILL_TOUCH_ALERT_ENABLED,
    DEFAULT_ISOLATION_TOUCH_LEVELS, DEFAULT_LEGACY_TOUCH_TELEGRAM_ENABLED,
    DEFAULT_LIVE_TOUCH_ALERT_ENABLED, DEFAULT_MAIN_INDEX_KEY,
    DEFAULT_TOUCH_ALERT_ENABLED, DEFAULT_TOUCH_ALERT_ONCE_PER_LEVEL,
    DEFAULT_TOUCH_ALERT_OPTIONS_ONLY, DEFAULT_TOUCH_CHECK_MODE
)
...
def get_default_touch_status() -> dict:
    ...
    return {
        "r2_touched": False, "s2_touched": False, "r3_touched": False, "s3_touched": False,
        "r2_touch_time": None, "s2_touch_time": None, "r3_touch_time": None, "s3_touch_time": None,
        "r2_alert_sent": False, "s2_alert_sent": False, "r3_alert_sent": False, "s3_alert_sent": False,
...
def create_touch_event(
    ...
    normalized_instrument_key = str(instrument_key or "").strip()
    normalized_level = str(level or "").strip().upper()
    normalized_contract_info = deepcopy(contract_info) if isinstance(contract_info, dict) else {}
...
def build_touch_status_from_events(events: list) -> dict:
    ...
    for event in events:
        ...
        status[f"{lower_level}_alert_sent"] = bool(event.get("telegram_alert_sent", event.get("alert_sent", False)))
...
def update_touch_status_in_cache(instrument_key: str, event: dict) -> bool:
    ...
    with runtime_state.opening_range_cache_lock:
        ...
        touch_status[f"{lower_level}_alert_sent"] = bool(event.get("telegram_alert_sent", event.get("alert_sent", False)))
...
def detect_touch_from_candle(
    ...
    if not instrument_key or not isinstance(candle, dict) or not isinstance(levels, dict):
        return []
    normalized_contract_info = contract_info if isinstance(contract_info, dict) else {}
    level_map = {
        "R2": {"value": safe_float(levels.get("r2")), "condition_field": "high", "trigger": safe_float(candle.get("high")), "fallback": safe_float(candle.get("close")), "direction": "above"},
        "R3": {"value": safe_float(levels.get("r3")), "condition_field": "high", "trigger": safe_float(candle.get("high")), "fallback": safe_float(candle.get("close")), "direction": "above"},
        "S2": {"value": safe_float(levels.get("s2")), "condition_field": "low", "trigger": safe_float(candle.get("low")), "fallback": safe_float(candle.get("close")), "direction": "below"},
        "S3": {"value": safe_float(levels.get("s3")), "condition_field": "low", "trigger": safe_float(candle.get("low")), "fallback": safe_float(candle.get("close")), "direction": "below"},
    }
    ...
    for level_name, level_details in level_map.items():
        ...
        if trigger_price <= 0:
            continue
        touched = trigger_price >= level_value if level_details["direction"] == "above" else trigger_price <= level_value
        if not touched:
            continue
        if should_skip_touch_alert(instrument_key=instrument_key, level=level_name, contract_info=normalized_contract_info):
            continue
...
def scan_backfill_touches(instrument_key: str, candles: list, levels: dict, contract_info: dict, candle_count: int) -> list:
    ...
    if not DEFAULT_BACKFILL_SCAN_ENABLED or not DEFAULT_BACKFILL_TOUCH_ALERT_ENABLED:
        return []
    if not instrument_key or not isinstance(candles, list) or not isinstance(levels, dict):
        return []
    runtime_state.ensure_current_market_day()
...
def extract_feed_values(tick_data: dict) -> dict:
    ...
    empty_result = {"ltp": 0.0, "high": 0.0, "low": 0.0, "close": 0.0, "timestamp": default_timestamp}
    if not isinstance(tick_data, dict):
        return empty_result
    raw_feed_object = _safe_dict(tick_data.get("raw_feed", tick_data))
    full_feed = _safe_dict(raw_feed_object.get("fullFeed", raw_feed_object))
    ...
    ltp = safe_float(ltpc.get("ltp"), default=0.0)
    ...
    matching_i1_candles = [
        item for item in ohlc_list
        if str(item.get("interval") or "").strip().upper() in {"I1", "1MINUTE", "1_MINUTE"}
    ]
    latest_i1 = None
    if matching_i1_candles:
        def get_ohlc_timestamp(item: dict) -> float:
        ...
        try:
            latest_i1 = max(matching_i1_candles, key=get_ohlc_timestamp)
        except Exception:
    ...
    if latest_i1 is not None:
        high_value = safe_float(latest_i1.get("high"), default=0.0)
        low_value = safe_float(latest_i1.get("low"), default=0.0)
