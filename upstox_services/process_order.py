...
def _send_telegram_message(
    ...
    """
    Send a Telegram message without interrupting the order workflow.
    """
...
def process_selected_instrument(
    ...
    instrument_details = _build_instrument_details(
        selected_instrument
    )

    trading_symbol = instrument_details.get(
        "trading_symbol"
    )
    instrument_key = instrument_details.get(
        "instrument_key"
    )
    live_ltp = instrument_details.get(
        "live_ltp"
    )
    lot_size = instrument_details.get(
        "lot_size"
    )
    ...
    if not trading_symbol or not instrument_key:
        logger.error(
            "Order workflow stopped. Required instrument "
            "details are missing. trading_symbol=%s, "
            "instrument_key=%s",
            trading_symbol,
            instrument_key,
        )
        ...
        _send_telegram_message(
            ...
            message=(
                "Required instrument details are missing.\n"
                f"Symbol: {trading_symbol}\n"
            ...
            level="ERROR",
            notification_context=(
                "process_order|invalid_instrument"
            ),
        )
        ...
        return _build_failure_result(
            ...
            error="Required instrument details are missing.",
        )

    logger.info(
        "Step 1 started. Exiting all existing positions."
    )
    ...
    try:
        ...
        return _build_failure_result(
            ...
            instrument_details=instrument_details,
            error="Position exit raised an exception.",
        )
    ...
    _send_telegram_message(
        ...
        level="SUCCESS",
        notification_context=(
            "process_order|exit_success|"
            f"instrument_key={instrument_key}"
        ),
    )

    logger.info(
        "Step 2 started. Placing new order. "
        "trading_symbol=%s, instrument_key=%s",
        trading_symbol,
        instrument_key,
    )
    ...
    _send_telegram_message(
        ...
        level="INFO",
        notification_context=(
            "process_order|placement_started|"
            f"instrument_key={instrument_key}"
        ),
    )
    ...
    try:
        ...
        _send_telegram_message(
            ...
            level="ERROR",
            notification_context=(
                "process_order|placement_exception|"
                f"instrument_key={instrument_key}"
            ),
        )
        ...
        return _build_failure_result(
            ...
            instrument_details=instrument_details,
            exit_result=exit_result,
            error="Order placement raised an exception.",
        )
    ...
    if place_order_result.get("success") is not True:
        ...
        _send_telegram_message(
            ...
            level="ERROR",
            notification_context=(
                "process_order|placement_failed|"
                f"instrument_key={instrument_key}"
            ),
        )
