...
def get_backfill_isolation_payload() -> dict:
    try:
        ...
        backfill_isolation = cache.get(
            "backfill_isolation",
            {},
        )

        if not isinstance(backfill_isolation, dict):
            backfill_isolation = {}

        return {
            "evaluated": bool(
                backfill_isolation.get(
                    "evaluated",
                    False,
                )
            ),
            ...
            "status": backfill_isolation.get(
                "status",
                "not_evaluated",
            ),
            "reason_code": backfill_isolation.get(
                "reason_code",
                "not_evaluated",
            ),
            "reason": backfill_isolation.get(
                "reason",
                (
                    "Backfill isolation has not been "
                    "evaluated for the current market day."
                ),
            ),
            "source": backfill_isolation.get("source"),
            "evaluated_instruments": backfill_isolation.get(
                "evaluated_instruments",
                0,
            ),
            "successful_instruments": backfill_isolation.get(
                "successful_instruments",
                0,
            ),
            "instruments_with_touch": backfill_isolation.get(
                "instruments_with_touch",
                0,
            ),
            ...
            "selected_instrument_key": (
                backfill_isolation.get("selected_instrument_key")
            ),
            "selected_level": backfill_isolation.get("selected_level"),
            "selected_at": backfill_isolation.get("selected_at"),
            "touch_time": backfill_isolation.get("touch_time"),
            "selection_reason": backfill_isolation.get("selection_reason"),
            "reference_average": backfill_isolation.get("reference_average"),
            "average_window": backfill_isolation.get(
                "average_window",
                {},
            ),
            "selected_diagnostic": (backfill_isolation.get("selected_diagnostic")),
            "diagnostics": backfill_isolation.get(
                "diagnostics",
                [],
            ),
            "error": backfill_isolation.get("error"),
        }

    except Exception as ex:
        logger.warning(
            "Could not retrieve backfill isolation payload. " "error=%s: %s",
            type(ex).__name__,
            ex,
        )
        ...
        return {
            ...
            "reason": ("Backfill isolation information is " "currently unavailable."),
            "source": None,
...
def get_alert_execution_mode() -> str:
    return (
        "background"
        if bool(getattr(config, "ALGO_APP_SEND_IN_BACKGROUND", True))
        else "synchronous"
    )
...
def get_local_alert_test_payload() -> dict:
    ...
    return {
        ...
        "production_delivery_blocked": bool(
            local_test_mode
            and getattr(config, "LOCAL_ALERT_PRODUCTION_DELIVERY_BLOCKED", True)
        ),
        ...
        "source_name": (
            str(
                getattr(
                    config, "LOCAL_ALERT_SOURCE_NAME", "option_feed_engine_local_test"
                )
            )
            if local_test_mode
            else None
...
def get_telegram_payload() -> dict:
    try:
        ...
        return {
            ...
            "delivery_mode": str(
                service_status.get("delivery_mode", get_alert_transport_mode())
            ),
            "local_test_mode": bool(service_status.get("local_test_mode", False)),
            "target_url_configured": bool(
                service_status.get("target_url_configured", False)
            ),
            "target_url": (
                service_status.get("target_url")
                if get_alert_transport_mode() == "local_test"
                else None
            ),
            "bot_token_configured": bool(
                service_status.get("bot_token_configured", False)
            ),
            "chat_id_configured": bool(service_status.get("chat_id_configured", False)),
            "timeout_seconds": service_status.get("timeout_seconds"),
            "isolated_instrument_alert_enabled": bool(
                getattr(
                    config, "OPENING_RANGE_ISOLATED_INSTRUMENT_NOTIFY_ENABLED", True
                )
            ),
            "isolated_ema_alert_enabled": bool(
                getattr(config, "EMA_ISOLATED_INSTRUMENT_TELEGRAM_ENABLED", True)
            ),
            "production_delivery_blocked": bool(
                service_status.get("production_delivery_blocked", False)
            ),
            "secrets_exposed": False,
        }
    except Exception as ex:
        logger.warning(
            "Could not retrieve Telegram service status. error=%s: %s",
            type(ex).__name__,
            ex,
        )
        return {
...
def get_ema_order_side_rule_payload() -> dict:
    return {
        ...
        "strike_selection": {
            ...
            "offsets": getattr(
                config, "EMA_ALERT_NEAREST_STRIKE_OFFSETS", [-50, 0, 50]
            ),
            "max_order_instruments": getattr(
                config, "EMA_ALERT_MAX_ORDER_INSTRUMENTS", 3
            ),
            "clamp_to_filter_range": getattr(
                config, "EMA_ALERT_ORDER_STRIKES_CLAMP_TO_FILTER_RANGE", True
            ),
        },
        "fallback": {
            "used_only_when_isolated_type_missing": True,
            "bullish_option_type": getattr(
                config, "EMA_ALERT_BULLISH_OPTION_TYPE", "CE"
            ),
            "bearish_option_type": getattr(
                config, "EMA_ALERT_BEARISH_OPTION_TYPE", "PE"
            ),
        },
    }
...
def get_budget_range_payload() -> dict:
    return {
        ...
        "use_suggested_order_side": bool(
            getattr(config, "EMA_ALERT_BUDGET_USE_SUGGESTED_ORDER_SIDE", True)
        ),
        "subscribed_only": bool(
            getattr(config, "EMA_ALERT_BUDGET_SUBSCRIBED_ONLY", True)
        ),
        "require_live_ltp": bool(
            getattr(config, "EMA_ALERT_BUDGET_REQUIRE_LIVE_LTP", True)
        ),
        "sort_mode": getattr(
            config, "EMA_ALERT_BUDGET_SORT_MODE", "nearest_to_budget_midpoint"
        ),
...
def get_algo_app_payload() -> dict:
    try:
        ...
        return {
            ...
            "transport_mode": service_status.get(
                "delivery_mode", get_alert_transport_mode()
            ),
            "local_test_mode": bool(service_status.get("local_test_mode", False)),
            "url_configured": bool(service_status.get("url_configured", False)),
            "effective_url": (
                service_status.get("effective_url")
                if get_alert_transport_mode() == "local_test"
                else None
            ),
            "production_url_configured": bool(
                service_status.get("production_url_configured", False)
            ),
            "local_url_configured": bool(
                service_status.get("local_url_configured", False)
            ),
            "auth_type": service_status.get("auth_type", "none"),
            "authentication_configured": bool(
                service_status.get("authentication_configured", False)
            ),
            ...
            "include_event_id": bool(
                getattr(config, "ALGO_APP_INCLUDE_EVENT_ID", True)
            ),
            "schema_version": getattr(config, "ALGO_APP_PAYLOAD_SCHEMA_VERSION", "1.0"),
            "source_name": getattr(
                config, "ALGO_APP_SOURCE_NAME", "option_feed_engine"
            ),
            "production_delivery_blocked": bool(
                service_status.get("production_delivery_blocked", False)
            ),
        ...
        logger.warning(
            "Could not retrieve Algo App service status. error=%s: %s",
            type(ex).__name__,
            ex,
        )
        return {
...
def get_delivery_flow_payload() -> dict:
    ...
    return {
        ...
        "telegram_destination": (
            "local_telegram_simulator"
            if transport_mode == "local_test"
            else "telegram_api"
        ),
        "algo_app_destination": (
            "local_algo_app_simulator"
            if transport_mode == "local_test"
            else "configured_algo_app_url"
        ),
        "production_delivery_blocked": bool(
...
def get_ema_alert_content_payload() -> dict:
    return {
        "include_level_name": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_LEVEL_NAME", True)
        ),
        "include_nifty_ltp": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_NIFTY_LTP", True)
        ),
        "include_ema_details": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_EMA_DETAILS", True)
        ),
        "include_nearest_instruments": bool(
            getattr(
                config, "EMA_ISOLATED_ALERT_INCLUDE_NEAREST_ORDER_INSTRUMENTS", True
            )
        ),
        "include_budget_instruments": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_BUDGET_INSTRUMENTS", True)
        ),
        "include_candle_close": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_CANDLE_CLOSE", True)
        ),
        "include_candle_low": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_CANDLE_LOW", True)
        ),
        "include_close_low_difference": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_CLOSE_LOW_DIFFERENCE", True)
        ),
        "include_candle_time": bool(
            getattr(config, "EMA_ISOLATED_ALERT_INCLUDE_CANDLE_TIME", True)
        ),
        "price_decimal_places": getattr(
            config, "EMA_ISOLATED_ALERT_PRICE_DECIMAL_PLACES", 2
        ),
    }
...
def get_ema_algo_payload_config() -> dict:
    return {
        "include_opening_range": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_OPENING_RANGE", True)
        ),
        "include_ema_values": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_EMA_VALUES", True)
        ),
        "include_candle": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_CANDLE", True)
        ),
        "include_nearest_instruments": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_NEAREST_INSTRUMENTS", True)
        ),
        "include_nearest_instrument_candles": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_NEAREST_INSTRUMENT_CANDLES", True)
        ),
        "include_budget_instruments": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_BUDGET_INSTRUMENTS", True)
        ),
        "include_budget_instrument_candles": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_BUDGET_INSTRUMENT_CANDLES", True)
        ),
        "include_delivery_metadata": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_DELIVERY_METADATA", False)
        ),
        "include_raw_ema_event": bool(
            getattr(config, "EMA_ALGO_PAYLOAD_INCLUDE_RAW_EMA_EVENT", True)
        ),
        "transport_mode": get_alert_transport_mode(),
    }
...
def resolve_opening_range_instrument_key(
    ...
    if option_type not in {"CE", "PE"}:
        raise HTTPException(
            status_code=400, detail="Invalid striketype. Allowed values: ce, pe"
        )
    try:
    ...
    raise HTTPException(
        status_code=404,
        detail=f"No option instrument found for strike={target_strike}, striketype={option_type}.",
    )
...
async def get_opening_range_latest_status():
    ...
    return {
        ...
        "backfill_isolation": (get_backfill_isolation_payload()),
...
async def get_opening_range_dashboard(
    ...
    backfill_isolation = summary.get("backfill_isolation")

    if not isinstance(backfill_isolation, dict):
        summary["backfill_isolation"] = get_backfill_isolation_payload()
...
async def trigger_opening_range_fetch(
    ...
    logger.info(
        ...
        "max_workers=%s, transport_mode=%s",
        selected_candle_count,
        selected_save_results,
        selected_max_workers,
...
async def fetch_opening_range_for_single_instrument(
    ...
    resolved_instrument_key = resolve_opening_range_instrument_key(
        instrument_key=instrument_key, strike=strike, striketype=striketype
    )
    selected_candle_count = (
        candle_count
        if candle_count is not None
        else getattr(config, "OPENING_RANGE_CANDLE_COUNT", 1)
    )
    try:
        ...
        error_message = f"{type(ex).__name__}: {ex}"
        logger.error(
            "Single instrument Opening Range fetch failed. instrument_key=%s, error=%s",
            resolved_instrument_key,
            error_message,
        )
        raise HTTPException(
            status_code=500,
            detail=f"Opening Range fetch failed for {resolved_instrument_key}: {error_message}",
        ) from ex
...
async def get_opening_range_full_cache():
    ...
    backfill_isolation = cache.get("backfill_isolation")
...
async def get_opening_range_instrument(
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail=f"Opening Range result not found for {resolved_instrument_key}.",
        )
    return {
        "status": "success",
        "instrument_key": resolved_instrument_key,
...
async def get_opening_range_pending_touch_events_route():
    return {
        "status": "success",
        "legacy_touch_telegram_enabled": bool(
...
async def flush_opening_range_pending_touch_events_route(
    ...
    try:
        sent = await run_in_threadpool(
            flush_pending_touch_alerts, force=force, source="manual_api_flush"
        )
        transport_mode = get_alert_transport_mode()
        return {
            "status": "success",
        ...
        raise HTTPException(
            status_code=500, detail=f"Touch alert flush failed: {error_message}"
        ) from ex
...
async def get_opening_range_file_status():
    opening_range_file = Path(
        getattr(config, "OPENING_RANGE_OUTPUT_FILE", "data/opening_range_results.json")
    )
    ...
    return {
        ...
        "config": {
            ...
            "opening_range_interval": getattr(
                config, "OPENING_RANGE_INTERVAL", "1minute"
            ),
            "opening_range_candle_count": getattr(
                config, "OPENING_RANGE_CANDLE_COUNT", 1
            ),
            ...
            "market_open_minute": getattr(
                config, "OPENING_RANGE_MARKET_OPEN_MINUTE", 15
            ),
            ...
            "backfill_touch_scan_enabled": getattr(
                config, "OPENING_RANGE_BACKFILL_TOUCH_SCAN_ENABLED", True
            ),
            "touch_alert_enabled": getattr(
                config, "OPENING_RANGE_TOUCH_ALERT_ENABLED", True
            ),
            "touch_alert_once_per_level": getattr(
                config, "OPENING_RANGE_TOUCH_ALERT_ONCE_PER_LEVEL", True
            ),
            "touch_alert_options_only": getattr(
                config, "OPENING_RANGE_TOUCH_ALERT_OPTIONS_ONLY", True
            ),
            "touch_check_mode": getattr(
                config, "OPENING_RANGE_TOUCH_CHECK_MODE", "high_low"
            ),
            "isolation_enabled": getattr(
                config, "OPENING_RANGE_ISOLATED_INSTRUMENT_ENABLED", True
            ),
            "isolation_window_points": getattr(
                config, "OPENING_RANGE_ISOLATION_AVERAGE_WINDOW_POINTS", 500.0
            ),
            ...
            "isolation_lock_for_day": getattr(
                config, "OPENING_RANGE_ISOLATION_LOCK_FOR_DAY", True
            ),
            "isolation_allow_priority_upgrade": getattr(
                config, "OPENING_RANGE_ISOLATION_ALLOW_PRIORITY_UPGRADE", True
            ),
            "isolation_allow_backfill_touch": getattr(
                config, "OPENING_RANGE_ISOLATION_ALLOW_BACKFILL_TOUCH", True
            ),
            "isolation_allow_live_touch": getattr(
                config, "OPENING_RANGE_ISOLATION_ALLOW_LIVE_TOUCH", True
            ),
            "isolated_notify_enabled": getattr(
                config, "OPENING_RANGE_ISOLATED_INSTRUMENT_NOTIFY_ENABLED", True
            ),
            "legacy_touch_telegram_enabled": getattr(
                config, "OPENING_RANGE_LEGACY_TOUCH_TELEGRAM_ENABLED", False
            ),
            "isolated_ema_telegram_enabled": getattr(
                config, "EMA_ISOLATED_INSTRUMENT_TELEGRAM_ENABLED", True
            ),
            "isolated_ema_every_cross": getattr(
                config, "EMA_ISOLATED_ALERT_EVERY_CROSS", True
            ),
            "ema_cross_include_opening_range": getattr(
                config, "EMA_CROSS_INCLUDE_OPENING_RANGE_LEVELS", True
            ),
            "ema_cross_broadcast_without_opening_range": getattr(
                config, "EMA_CROSS_BROADCAST_WITHOUT_OPENING_RANGE", True
            ),
            ...
            "live_ema_interval_minutes": getattr(
                config, "LIVE_EMA_INTERVAL_MINUTES", 1
            ),
            "live_ema_fast_period": getattr(config, "LIVE_EMA_FAST_PERIOD", 9),
            "live_ema_slow_period": getattr(config, "LIVE_EMA_SLOW_PERIOD", 21),
            "local_alert_test_mode": bool(
                getattr(config, "LOCAL_ALERT_TEST_MODE", False)
            ),
            "local_alert_production_delivery_blocked": bool(
                getattr(config, "LOCAL_ALERT_TEST_MODE", False)
                and getattr(config, "LOCAL_ALERT_PRODUCTION_DELIVERY_BLOCKED", True)
            ),
...
__all__ = [
    "router",
    "get_backfill_isolation_payload",
