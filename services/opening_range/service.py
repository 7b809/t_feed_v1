...
def _normalize_max_workers(
    ...
    """
    Returns a valid worker count.

    When the total number of instruments is known, the worker count is
    limited so unnecessary worker threads are not created.
    """
...
def _get_shared_runtime_snapshots() -> tuple[dict, dict, list]:
    ...
    Individual locks are used internally by state.py. No multiple locks
    remain held at the same time.
    """
...
def calculate_opening_range_for_instrument(
    ...
    """
    Fetches intraday candles for one instrument and calculates its
    Opening Range levels.

    The function also scans post-Opening-Range candles for historical
    R2, R3, S2, and S3 touches.
    """
    ...
    if (
        normalized_instrument_key == runtime_state.DEFAULT_MAIN_INDEX_KEY
        if hasattr(
            runtime_state,
        ...
        return {
            **base_payload,
            ...
            "message": (
                ...
                f"candles, but only "
                f"{len(selected_candles)} are available."
            ),
...
def calculate_opening_range_for_all_subscribed(
    ...
    """
    Fetches intraday candles and calculates Opening Range levels for all
    subscribed instruments.

    Backfill touch events are collected from all instruments before
    isolated-instrument selection. This allows higher-priority R3 or S3
    events to win over R2 or S2 events.
    """
    ...
    logger.info(
        ...
        "date=%s, candle_count=%s, interval=%s, "
        "unit=%s, intraday_interval=%s, "
        "market_open=%02d:%02d, max_workers=%s",
        total_instruments,
        current_date,
    ...
    with ThreadPoolExecutor(max_workers=normalized_max_workers) as executor:
        ...
        for future in as_completed(future_to_instrument):
            ...
            try:
                ...
                # Do not call queue_touch_event() here.
                # scan_backfill_touches() already queues each event.
    ...
    # Store all per-instrument results before isolated-instrument
    # selection. isolation.py reads range and level data from this cache.
    ...
    # Select one isolated instrument only after all backfill events have
    # been collected. This allows configured R3/S3 priority to win over
    # R2/S2 events from earlier worker completions.
    ...
    # Synchronize cache counters again because isolation and Telegram
    # processing may have changed state after the initial cache update.
