...
class TelegramTokenBot:
    ...
    def _send_manual_isolation_result(
        ...
        """
        Send the service result back to the Telegram
        chat that initiated the isolation.
        """

        if not isinstance(result, dict):
            logger.error(
                "Manual instrument isolation returned invalid result type from Telegram. type=%s, result=%s",
        ...
        if success:
            message = (
                ...
                f"Strike: {self._format_strike_price(strike_price)}\n"
                f"Option Type: {option_type}\n"
                f"Requested By: {requested_by}\n"
                "Source: telegram\n\n"
                "Service Result:\n"
                f"{self._format_json_for_telegram(result)}"
            )
        ...
        message += "\n\nService Result:\n" + self._format_json_for_telegram(result)
        self._send_bot_message(chat_id, message)
